...
Begin VB.Form FrmAjusDiferxCan 
   BackColor       =   &H00C0FFFF&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Ajuste de Difer x Cambio Cancelados"
   ClientHeight    =   2460
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   4005
   ...
   End
End
Attribute VB_Name = "FrmAjusDiferxCan"
...
On Error GoTo Ajuste
    ...
    VGCommandoSP.CommandText = "Ct_AjusDifCambCanc_pro"
    ...
    With VGCommandoSP
        ...
        .Parameters("@AjusteDebe") = Trim$(VGParametros.ajustedebe)
        .Parameters("@AjusteHaber") = Trim$(VGParametros.ajustehaber)
        .Parameters("@Fecha") = DTPicker1.Value
        .Parameters("@Usuario") = VGParamSistem.Usuario
        .Parameters("@NombrePC") = VGComputer
        .Parameters("@TipoCambio") = tipocambio
...
Private Sub EliminaAjustes()
    ...
    SQL = "Delete From ct_cabcomprob" & Year(DTPicker1.Value) & " Where empresacodigo='" & VGParametros.empresacodigo & "' " & _
        ...
        " AND cabcomprobnumero='" & Month(DTPicker1.Value) & "05500001' " & _
    ...
    SQL = "Delete From ct_cabcomprob" & Year(DTPicker1.Value) & " Where empresacodigo='" & VGParametros.empresacodigo & "' " & _
        "And cabcomprobmes=" & Month(DTPicker1.Value) & _
        " AND cabcomprobnumero='" & Month(DTPicker1.Value) & "05500002' " & _
        " AND subasientocodigo='0099' AND asientocodigo='055'"
    Set rs1 = VGCNx.Execute(SQL)
    Set rs1 = Nothing
End Sub
...
Set VGCommandoSP = New ADODB.Command
    ...
    End With
End Sub
Private Function tipocambio() As Double
    Dim rst As ADODB.Recordset
    Dim SQL As String
    Set rst = New ADODB.Recordset
    tipocambio = 1
    SQL = "Select tipocambiopromedio From ct_tipocambio Where tipocambiofecha='" & Format(DTPicker1.Value, "dd/MM/yyyy") & "'"
    Set rst = VGCNx.Execute(SQL)
    If Not rst.EOF Then tipocambio = rst(0)
    Set rst = Nothing
End Function
