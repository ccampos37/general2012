...
Begin VB.Form frmMantPlanCuentas 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      Begin TrueOleDBGrid70.TDBGrid TDBGrid1 
         ...
         Columns(9).DataField=   "cuentaestadoccostos"
         ...
         Columns(17).Caption=   "Adiciona Cargo"
         Columns(17).DataField=   "cuentaadicionacargo"
...
Sub EditarValores()
 Dim i As Integer
  With TDBGrid1
    ...
    chk(5).Value = IIf(Trim(.Columns(17).Text) = 1, 1, 0)
...
Sub ConfiguraModoEdicion()
    ...
    Else
        ...
        If lblNivel.Caption = VGnumnivelescuenta Then
...
Sub GrabarData()
  ...
  If modoinsert = True Then
    VGCNx.BeginTrans
    SQL = GrabarPlanCuenta(0, txt(0).Text, txt(1).Text, Val(txt(2).Text), Val(txt(3).Text), Val(txt(5).Text), txt(4).Text, txt(6).Text, chk(1).Value, chk(0).Value, chk(2).Value, CInt(lblNivel.Caption), Ctr_Ayuda1.xclave, chk(3).Value, Ctr_Ayuda4.xclave, Left(cboTipoAjuste.List(cboTipoAjuste.ListIndex), 2), chk(5).Value)
    VGCNx.Execute (SQL)
    
    If CInt(lblNivel.Caption) = VGnumnivelescuenta And FLAGDISTRIBUCION = True Then
  ...
  ElseIf modoedit = True Then
    VGCNx.BeginTrans
    SQL = GrabarPlanCuenta(1, txt(0).Text, txt(1).Text, Val(txt(2).Text), Val(txt(3).Text), Val(txt(5).Text), txt(4).Text, txt(6).Text, chk(1).Value, chk(0).Value, chk(2).Value, CInt(lblNivel.Caption), Ctr_Ayuda1.xclave, chk(3).Value, Ctr_Ayuda4.xclave, Left(cboTipoAjuste.List(cboTipoAjuste.ListIndex), 2), chk(5).Value)
    VGCNx.Execute (SQL)
 ...
 Dim SQL As String
  ...
  If lblNivel.Caption - 2 < 0 Then
    ...
    '    Exit Function
      End If
  Else
    If VG_aNIVELES(lblNivel.Caption - 1) < CLng(Len(txt(0).Text)) Or VG_aNIVELES(lblNivel.Caption - 2) >= CLng(Len(txt(0).Text)) Then
        MsgBox "La Cuenta a registrar no corresponde con el Nivel de Cuenta", vbInformation, Caption
        ValidarData = False
        txt(0).SetFocus
    '    Exit Function
  ...
  SQL = "SELECT cuentacodigo FROM CT_CUENTA WHERE cuentacodigo='" & txt(0).Text & "' and empresacodigo='" & VGParametros.empresacodigo & "'"
...
Private Sub txt_Change(Index As Integer)
  cAcepta.Enabled = IIf(xdllgen.ESNULO(lblNivel.Caption, 0) = VGnumnivelescuenta, ValidarCuentaUltimoNivel(), ValidarCuentaNivel())
End Sub
 ...
 Dim sw As Boolean
    ...
    If modoinsert = True And Index = 0 Then
      For i = 1 To VGnumnivelescuenta
       If i > 1 Then
        sw = VG_aNIVELES(i - 1) >= Len(Trim(txt(0).Text)) And VG_aNIVELES(i - 2) < Len(Trim(txt(0).Text))
       Else
        sw = VG_aNIVELES(i - 1) = Len(Trim(txt(0).Text))
       End If
       If sw Then
         lblNivel.Caption = i
         Call ConfiguraModoEdicion
...
Function GrabarCuentaDistribucion()
 ...
 If rsDist.RecordCount > 0 Then
   SQL = "DELETE FROM ct_distribucion WHERE empresacodigo='" & VGParametros.empresacodigo & "' and cuentacodigo='" & txt(0).Text & "'"
   VGCNx.Execute (SQL)
   rsDist.MoveFirst
...
Private Sub CargaGridDist()
  ...
  SQL = SQL & "FROM ct_distribucion WHERE empresacodigo='" & VGParametros.empresacodigo & "' and cuentacodigo='" & txt(0).Text & "'"
...
Function DevuelveTipoCuenta()
 ...
 SQL = "Select tipocuentacodigo from ct_cuenta where empresacodigo='" & VGParametros.empresacodigo & "' and cuentacodigo=left('" & Trim(txt(0).Text) & "',2)"
