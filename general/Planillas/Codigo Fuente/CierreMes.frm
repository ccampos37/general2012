...
If Len(xMes.Tag) > 0 Then
    If Len(XMESSTRING) > 0 Then
        If MsgBox("REALMENTE DESEA CERRAR " & XMESSTRING & " LA PLANILLA SELECCIONADA EN LA LISTA ?", vbYesNo, "CONFIRMACIÓN") = vbYes Then
            DBSYSTEM.Execute "UPDATE NOMBOL SET CERRADO=1 WHERE NOMBRE='" & XMESSTRING & "' AND FECHAINI=" & DateSQL(xFechaIni.Value) & " AND FECHAFIN=" & DateSQL(xFechaFin.Value) & "", X
            If X > 0 Then
                MsgBox "LA PLANILLA " & XMESSTRING & " FUE CLAUSURADA SATISFACTORIAMENTE", vbInformation, "CLAUSURA DE MES"
            End If
        End If
    Else
        MsgBox "SELECCIONE UNA PLANILLA DE LA LISTA DEL MES DE " & Format(xMes.Tag, "MMMM"), vbInformation
        Exit Sub
    End If
Else
    MsgBox "SELECCIONE EL MES A CERRAR", vbInformation
    xMes.SetFocus
    Exit Sub
End If
End Sub
