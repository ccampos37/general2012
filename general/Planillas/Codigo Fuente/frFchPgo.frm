...
Public Sub CargaCrono(ByVal vCosto As String)
    ...
    RsAux.Open "SELECT * FROM CCostos WHERE Cronograma=1 AND CodCCosto='" & vCosto & "'", DbSystem, adOpenStatic
    If RsAux.EOF Or RsAux.RecordCount = 0 Then
        MsgBox "El centro de costo seleccionado no se encuentra configurado para aceptar Cronograma de Pagos de Remuneraciones", vbInformation
        Unload Me
        Exit Sub
    End If
    ...
    Set RsAux = Nothing
    xFechaIni_CloseUp
    RsCrono.Open "SELECT * FROM FechaPago WHERE CodCCosto='" & vCosto & " ' ORDER BY FechaIni", DbSystem, adOpenKeyset
...
Private Sub cmAgregar_Click()
    If xDescripcion.Text = "" Then
        ...
        Exit Sub
    End If
    DbSystem.Execute "INSERT INTO FechaPago (CodCCosto, FechaIni, FechaFin, Adelanto, FechaPago, Nombre) VALUES ('" & xCCosto.Tag & "', " & DateSQL(xFechaIni.Value) & "," & DateSQL(xFechaFin.Value) & ", " & DateSQL(xAdelanto.Value) & "," & DateSQL(xFechaPago.Value) & ",'" & xDescripcion.Text & "')"
    RsCrono.Requery
...
Private Sub cmBorrar_Click()
    ...
    DbSystem.Execute "DELETE FROM FechaPago WHERE ID_FechaPago=" & RsCrono!ID_FechaPago
    RsCrono.Requery
    FormatDG
End Sub
...
Private Sub cmLimpiar_Click()
    ...
    DbSystem.Execute "DELETE FROM FechaPago WHERE CodCCosto='" & xCCosto.Tag & "'"
    RsCrono.Requery
    FormatDG
End Sub
