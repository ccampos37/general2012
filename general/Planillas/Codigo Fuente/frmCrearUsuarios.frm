...
Select Case INDEX
 ...
 Case 1: 'GRABAR
         ...
         If NTIPO = 1 Then
            ...
            If Not FLAG Then
                ...
                ElseIf Text1(2).Text = Text1(3).Text Then
                    ...
                    ADOREG2.Fields("EMP_CODIGO") = REGSISTEMA.RUC
                    ADOREG2.Fields("USU_PASSWORD") = PROCSIS.PrCodifica(Trim(Text1(2).Text), NUMMAGICO) 'PASSWORD                    ADOREG2.UPDATEBATCH
        ...
        If NTIPO = 2 Then
            ADOREG2.Fields("USU_CODIGO") = UCase(Trim(Text1(0).Text))
            ADOREG2.Fields("EMP_CODIGO") = REGSISTEMA.RUC
            ADOREG2.Fields("USU_PASSWORD") = PROCSIS.PrCodifica(Trim(Text1(2).Text), NUMMAGICO)
            If Trim(Text1(1).Text) <> "" Then ADOREG2.Fields("USU_NOMBRE") = UCase(Trim(Text1(1).Text))
            ADOREG2.UpdateBatch
            ADOREG1.Requery
            ADOREG2.Requery
 ...
 Case 2: 'EDITAR
         If ADOREG1.Bookmark Then
            ...
            Text1(2).Text = PROCSIS.PrDecodifica(ADOREG2.Fields(2), NUMMAGICO)
            Text1(3).Text = PROCSIS.PrDecodifica(ADOREG2.Fields(2), NUMMAGICO)
            If Not IsNull(ADOREG2.Fields("USU_NOMBRE")) Then Text1(1).Text = ADOREG2.Fields("USU_NOMBRE")
        
            Call EDIT_MEN(ADOREG2.Fields(0), REGSISTEMA.RUC)
'            ME.CLS
'            CALL EDIT_MEN(ADOREG2.FIELDS(0), VGEMP_CODIGO)
 ...
 Case 3: 'ELIMINAR
          ...
          If OP = vbYes Then
            ADOREG2.Bookmark = ADOREG1.Bookmark
            DBSTARPLAN.Execute "DELETE FROM " & CLMENU.TabaMenuDet & " WHERE USU_CODIGO = '" & ADOREG1("USU_CODIGO") & "' AND EMP_CODIGO = '" & REGSISTEMA.RUC & "'"
            ADOUSU.Requery
            ADOREG2.Delete
            ADOREG2.UpdateBatch
            ADOREG2.Requery
...
Private Sub Form_Load()
    ...
    XFLAG = True
    Me.Caption = "USUARIOS - " & REGSISTEMA.EMPRESA
...
ADOREG1.Open "SELECT USU_CODIGO,USU_NOMBRE FROM " & CLMENU.TablaUsu & " WHERE EMP_CODIGO=" & "'" & REGSISTEMA.RUC & "'", DBSTARPLAN, adOpenStatic
ADOREG2.Open "SELECT * FROM " & CLMENU.TablaUsu & " WHERE EMP_CODIGO=" & "'" & REGSISTEMA.RUC & "'", DBSTARPLAN, adOpenDynamic, adLockOptimistic
ADOUSU.Open "SELECT * FROM " & CLMENU.TabaMenuDet, DBSTARPLAN, adOpenDynamic, adLockOptimistic
...
ADOMEN.Open "SELECT * FROM " & CLMENU.TablaMenu & " ORDER BY MEN_CODIGO", DBSTARPLAN, adOpenStatic
...
Do While Not ADOMEN.EOF
    If TreeView1.Nodes(1).Checked Then  'RAIZ
        If TreeView1.Nodes.Item(NII).KEY = ADOMEN("MEN_CODIGO") & " ID" Then
            ...
            NOP = 2
            ADOUSU.Filter = "USU_CODIGO = '" & CCOD & "' AND EMP_CODIGO = '" & REGSISTEMA.RUC & "' AND MEN_CODIGO = '" & ADOMEN("MEN_CODIGO") & "'"
            ...
            ADOUSU("EMP_CODIGO") = REGSISTEMA.RUC
...
FrameU(0).Visible = False
FrameU(1).Visible = True
ADOUSME.Open "SELECT MEN_CODIGO,MEN_HAB FROM " & UCase(CLMENU.TabaMenuDet) & " WHERE USU_CODIGO = '" & CCODU & "' AND EMP_CODIGO = '" & CCODE & "'", DBSTARPLAN, adOpenStatic
