...
NII = 2
ADOUSU.Requery
If ADOMEN.RecordCount > 0 Then ADOMEN.MoveFirst
Do While Not ADOMEN.EOF
    If TreeView1.Nodes(1).Checked Then  'RAIZ
        If TreeView1.Nodes.Item(NII).Key = ADOMEN("MEN_CODIGO") & " ID" Then
            ...
            NOP = 2
            ADOUSU.Filter = "USU_CODIGO = '" & cCod & "' AND EMP_CODIGO = 'CO' AND MEN_CODIGO = '" & ADOMEN("MEN_CODIGO") & "'"
            ...
            ADOUSU("EMP_CODIGO") = "CO" ' VGParametros.RucEmpresa
...
FrameU(0).Visible = False
FrameU(1).Visible = True
ADOUSME.Open "SELECT MEN_CODIGO,MEN_HAB FROM " & UCase(CLMENU.TabaMenuDet) & " WHERE USU_CODIGO = '" & CCODU & "' AND EMP_CODIGO = 'CO'", VGCNx, adOpenStatic
