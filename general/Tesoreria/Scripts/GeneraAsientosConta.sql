...
exec marfice_ventas.dbo.te_GeneraTempContable @Baseventa,@Baseconta,'776101','976101','Desarrollo3','12','2002','E'
...
'If Exists(Select name from tempdb..sysobjects where name=''##tmpgenasientocab'+@compu+''') 
 ...
 Select numprovi=A.cabrec_numrecibo,
        ...
        cabcomprobfeccontable=cast (''01/01/2000'' as datetime) ,
...
Set @SqlCad='
 ...
 (select numprovi=A.cabrec_numrecibo,
       ...
       detcomprobtipocambio=A.TipoCambio, 
		 detcomprobruc=space(11),
...
EXEC te_GeneraAsientosTesoreria_pro 'VENTAS_PRUEBA','CONTAPRUEBA','012','0001','01','12','2002','001','Desarrollo3','TESOR01'
...
/*Delete From contaprueba..ct_cabcomprob2002 
  Where cabcomprobmes=12 and asientocodigo='012' 

select * From contaprueba..ct_detcomprob2002 
  Where cabcomprobmes=12 and asientocodigo='012' 
*/
...
--isnull(Case when A.monedacodigo=''02'' then importeabono else A.cargo /tccancela end,0) ,
--   detcomprobussdebe=isnull(HaberD,0),   /*isnull(Case when A.monedacodigo=''02'' then importeabono else A.abono /tccancela end,0) ,

/*
select * from contaprueba.dbo.ct_cabcomprob2002 
where asientocodigo='012' and cabcomprobmes=12
order by 1,2
...
select cabcomprobmes,cabcomprobnumero,sum(detcomprobdebe)-sum(detcomprobhaber)
from contaprueba.dbo.ct_detcomprob2002 
where asientocodigo='012' and cabcomprobmes=12
group by cabcomprobmes,cabcomprobnumero
having sum(detcomprobdebe)-sum(detcomprobhaber)<>0
*/