...
If rs.RecordCount > 0 Then
   Limpiar
    resp = "N"
    Frame3.Caption = "Modificación de Titulo de Tela"
    DBGrid1.Visible = False
    'Command19.Visible = False
    Frame2.Visible = False
    Frame5.Visible = False
...
If resp = "S" Then
  If Trim(Text1) = "" Then
     MsgBox "Ingrese Código de titulo de Tela", vbInformation, "Mensaje"
     Text1.SetFocus
     Exit Sub
  Else
       If VGDllGeneral.VerificaDatoExistente(VGCNx, "select * from titulo where CODIGO='" & Trim(Text1) & "'") = 1 Then
          MsgBox "El código de titulo de tela ya existe", vbInformation, "Mensaje"
          Text1.SetFocus
          Exit Sub
       End If
  End If
End If
  ...
  If Trim(Text2) = "" Then
     MsgBox "Ingrese Descripción de titulo de Tela", vbExclamation, "Aviso"
     Text2.SetFocus
     Exit Sub
  End If
