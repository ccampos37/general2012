...
Begin VB.Form FrmModificar 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin MSFlexGridLib.MSFlexGrid MsFlexGrid1 
         ...
         Rows            =   1
         Cols            =   11
         FormatString    =   $"FrmModificar.frx":1D12A
      End
   ...
   Begin VB.Frame Frame3 
      ...
      Begin VB.CommandButton Command6 
         ...
         Height          =   735
         Left            =   480
         Picture         =   "FrmModificar.frx":1D20E
         Style           =   1  'Graphical
         TabIndex        =   32
         Top             =   120
         Width           =   840
      ...
      Begin VB.CommandButton Command5 
         ...
         Height          =   735
         Left            =   1800
         Picture         =   "FrmModificar.frx":1D518
         Style           =   1  'Graphical
         TabIndex        =   31
         Top             =   120
         Width           =   840
      ...
      Begin VB.CommandButton CmdEliminar 
         ...
         Height          =   735
         Left            =   3075
         Picture         =   "FrmModificar.frx":1D95A
         Style           =   1  'Graphical
         TabIndex        =   29
         Top             =   120
         Width           =   900
      ...
      Begin VB.CommandButton Cmdgrabar 
         ...
         Height          =   735
         Left            =   3075
         Picture         =   "FrmModificar.frx":1DD9C
         Style           =   1  'Graphical
         TabIndex        =   30
         Top             =   120
         Visible         =   0   'False
   ...
   Begin VB.CommandButton Command7 
      ...
      Height          =   735
      Left            =   5685
      Picture         =   "FrmModificar.frx":1E1DE
      Style           =   1  'Graphical
      TabIndex        =   25
      Top             =   4680
      Width           =   885
   ...
   Begin VB.CommandButton Command1 
      ...
      Height          =   735
      Left            =   3195
      Picture         =   "FrmModificar.frx":1E620
      Style           =   1  'Graphical
      TabIndex        =   24
      Top             =   4680
      Width           =   855
   ...
   Begin VB.Frame Frame4 
      ...
      Begin VB.ComboBox Combo1 
         Height          =   315
         ItemData        =   "FrmModificar.frx":1EA62
         Left            =   4440
         List            =   "FrmModificar.frx":1EA6F
...
'FrmCreacionSin de alli graba  el detalle en la tabla,la cabecera lo graba en
'form de modifficar
...
'  End If
  ...
  rsstock.Open RSQL, VGCNx, adOpenDynamic, adLockOptimistic
  '*****************************************
  entrodetalle = True
  muestradetalle
 ...
 Dim precio As Double
  ...
  If Frame2.Visible Then
        If Option3.Value Then
         ...
         Else
             tipo = "XX"
             RSQL = "select  m.caalma,m.CATD, m.CANUMDOC, m.CACODMOV ,m.CAFECDOC, m.CACODPRO,m.CACODCLI, m.CARFTDOC ,m.CARFNDOC,m.CACIERRE from MovAlmCab m where m.CASITGUI<>'A' and m.CAALMA ='" & VGAlma & "'   and m.CATD  IN  ('NI','NS','GS')  and  m.cafecdoc  between " & DateSQL(DTPicker2.Value) & " and " & DateSQL(DTPicker3.Value) & "  ORDER BY m.CANUMDOC"    '
         End If
         If tipo <> "XX" Then
           ...
           RSQL = "select m.caalma,m.CATD, m.CANUMDOC, m.CACODMOV ,m.CAFECDOC, m.CACODPRO,m.CACODCLI, m.CARFTDOC ,m.CARFNDOC,m.CACIERRE " & _
               "from MOVALMCAB m where m.CASITGUI<>'A' and m.CATD='" & tipo & "' and  isnull(m.CACIERRE,0)=0  AND m.cafecdoc  between '" & DTPicker2.Value & "' and '" & Format(DTPicker3.Value) & "' ORDER BY m.caalma,m.CANUMDOC"
        ...
        While Not Adoreg1.EOF
            FG.AddItem (Adoreg1(0) & vbTab & Adoreg1(1) & vbTab & Adoreg1(2) & vbTab & Adoreg1(3) & vbTab & Adoreg1(4) & vbTab & Adoreg1(5) & vbTab & Adoreg1(6) & vbTab & Adoreg1(7) & vbTab & Adoreg1(8) & vbTab & IIf(Adoreg1(9), "*", " "))
            Adoreg1.MoveNext
        Wend
        Adoreg1.Close
  ...
  Else
     ...
     If Trim(FG.TextMatrix(FG.Row, 4)) <> "" Then
         DTPicker1 = Format(FG.TextMatrix(FG.Row, 4), "dd/mm/yyyy") 'fecha
         Fec1 = Format(FG.TextMatrix(FG.Row, 4), "yyyy") + Format(FG.TextMatrix(FG.Row, 4), "mm")
     End If
     TxTransa = FG.TextMatrix(FG.Row, 3)  'tras
     lbltransa = transa
     TxDoc = FG.TextMatrix(FG.Row, 1) ' tipo de doc
     If TxTransa = "TD" Then
         MsgBox "No se puede modificar el documento por tipo de transacion", vbInformation, "Aviso"
         Exit Sub
     End If
     TxAlmDes = FG.TextMatrix(FG.Row, 0)
     VGAlma = TxAlmDes
     Command1.Visible = False
     Frame1.Visible = True
     Frame3.Visible = True
     Command1.Caption = "&Aceptar"
     Lblnumdoc = FG.TextMatrix(FG.Row, 2) ' cod de doc
     Ctr_AyuProveedor.xclave = FG.TextMatrix(FG.Row, 5): Ctr_AyuProveedor.Ejecutar    'proveedor
     TxClie = FG.TextMatrix(FG.Row, 6)
     If TxClie <> "" Then lblClie = existe_clie(TxClie)
     If lblClie <> "" Then lblClie = Mid(lblClie, 1, 18)
     TxDocRef = FG.TextMatrix(FG.Row, 7)  'doc ref
     TxDocNroRef = FG.TextMatrix(FG.Row, 8)  'proveedor
     '*RMM*********************************************
     tipo = FG.TextMatrix(FG.Row, 1)
     '*RMM*********************************************
     ...
     If Adoreg1.RecordCount <> 0 Then
        ...
        Ctr_AyuProveedor.xclave = IIf(IsNull(Adoreg1(4)), "", Adoreg1(4))
...
Function transa() As String
 ...
 Dim RSQL As String
'  VGAlma = "01"
  RSQL = "select  TT_DESCRI FROM TabTransa where TT_CODMOV= '" & FG.TextMatrix(FG.Row, 3) & "'" '
 ...
 Dim Adoreg1 As ADODB.Recordset
  ...
  On Error GoTo GrabErr
   If TxDoc.text <> "" Then
        ...
        adodc1.Open "Select * From MovAlmCab Where CANUMDOC = '" & FG.TextMatrix(FG.Row, 2) & "' And  CATD = '" & tipo & "' And CAALMA = '" & TxAlmDes & "'  ", VGCNx, adOpenDynamic, adLockOptimistic
        If adodc1.RecordCount > 0 Then
          adodc1("CAFECDOC") = DTPicker1.Value
          FG.TextMatrix(FG.Row, 4) = DTPicker1.Value          'corregir
          TipoDeCambio = 0
          ...
          If Trim(Ctr_AyuProveedor.xclave) <> "" Then
             adodc1("CACODPRO") = Trim(Ctr_AyuProveedor.xclave)
          Else
             adodc1("CACODPRO") = " "
          End If
...
'************************************
  ...
  Set rsstock = New ADODB.Recordset
  rsstock.Open "Select * from stkart", VGCNx, adOpenDynamic, adLockOptimistic
...
Dim n As String
     ...
     If Adoreg1.RecordCount = 0 Then
         ...
         Exit Sub
     End If
     If TipoDeCambio = 0 And Text1 = "02" Then
     ...
     While Not Adoreg1.EOF
        ...
        '*RMM***********************************************************************************************************************************************
        'MSFlexGrid1.AddItem (Adoreg1("DECODIGO") & vbTab & Adoreg1("DEDESCRI") & vbTab & ser_lot & vbTab & Format(Adoreg1(3), "##0.00") & vbTab & Adoreg1(2) & vbTab & Format(IIf(Text1 = "02", Adoreg1("DEPRECIO") / IIf(TipoDeCambio <> 0, TipoDeCambio, 1), Adoreg1("DEPRECIO")), "####0.0000") & vbTab & Adoreg1(7) & vbTab & "" & vbTab & "" & vbTab & "" & vbTab & dato & vbTab)
...
Private Sub TxCenCos_DblClick()
  ...
  Else
        Adodc3.Open "SELECT cencost_codigo,cencost_descripcion FROM centro_costos where  len(cencost_codigo) = '6' ", VGCNx, adOpenStatic, adLockOptimistic
  End If
...
Function transa1(text As TextBox) As String
 Dim adoreg As ADODB.Recordset
  ...
  If Trim(tipo) = UCase("xx") Then
     adoreg.Open "select  TT_DESCRI FROM TabTransa where TT_CODMOV= '" & TxTransa & "'", VGCNx, adOpenDynamic, adLockOptimistic
  Else
      adoreg.Open "select  TT_DESCRI FROM TabTransa where TT_CODMOV= '" & TxTransa & "' and TT_TIPMOV ='" & IIf(tipo = "NI", "I", "S") & "'", VGCNx, adOpenDynamic, adLockOptimistic
  End If
...
Private Sub Inicializa()
    FG.FormatString = "Alm.|Tipo Doc.|Numero de Doc| Tr| Fecha | Proveedor|Cliente|Td REF|Num.Doc Ref."
    FG.Row = 0
    FG.Cols = 10
    FG.ColWidth(0) = 400
    FG.ColWidth(1) = 400
