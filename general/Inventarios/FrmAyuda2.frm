...
Begin VB.Form FrmAyuda2 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.ComboBox Combo1 
         Height          =   315
         ItemData        =   "FrmAyuda2.frx":0000
         Left            =   270
         List            =   "FrmAyuda2.frx":0002
...
Dim flgVentanaActivada As Boolean

Private Sub cAcepto_Click()
  Call DGrid0_KeyDown(13, 0)
End Sub
...
Private Sub DGrid1_HeadClick(ByVal ColIndex As Integer)
  ...
  ConfigGrid xtipo
End Sub
Private Sub DGrid0_KeyDown(KeyCode As Integer, Shift As Integer)
 If KeyCode = 13 Then
    If Val(DGrid1.Columns(0).text) >= 0 Then
        Select Case xtipo
          Case 1, 3
            ...
            If DGrid1.Columns.count = 3 Then
               If xtipo = 1 Then
                  ndensidad = Val(DGrid1.Columns(2).text)
                  nSaldo = Val(DGrid1.Columns(2).text)
                Else
                  nSaldo = Val(DGrid1.Columns(2).text)
               End If
            End If
            If DGrid1.Columns.count = 4 Then
               nSaldo = Val(DGrid1.Columns(2).text)
               ndensidad = Val(DGrid1.Columns(3).text)
            End If
...
Private Sub Form_Activate()
    If Not flgVentanaActivada Then
        flgVentanaActivada = True
        Text1.SetFocus
    End If
End Sub

Private Sub Form_Load()
    flgVentanaActivada = False
  central Me
  nlongi(1) = 0
  DoEvents
  ...
  ElseIf Trim(Escadena(xdata)) = "2" Then
     Call DLLGENERALAYUDA.ListarEnTDBGRID(vcon, xtabla, DGrid1, xCampos, xOrden, nlongi, "adescri2 like '" & xdato & "%' and stalma='" & FrmTraslado.Ctr_Ayuda1.xclave & "' AND STSKDIS>0")
     Text1 = Escadena(xdato): Text1.SelStart = Len(Trim(Text1))
'     cTraslado.Visible = True
 ...
 ElseIf Trim(Escadena(xdata)) = "4" Then
     ...
     Text1 = Escadena(xdato): Text1.SelStart = Len(Trim(Text1))
     Call Text1_KeyPress(13)
   ElseIf Trim(Escadena(xdata)) = "5" Then
     Call DLLGENERALAYUDA.ListarEnTDBGRID(vcon, xtabla, DGrid1, xCampos, xOrden, nlongi, "adescri2 like '" & xdato & "%' and stalma='" & FrmDistribucion.Ctr_Ayuda1.xclave & "' AND STSKDIS>0")
     Text1 = Escadena(xdato): Text1.SelStart = Len(Trim(Text1))
     Call Text1_KeyPress(13)
  ElseIf Trim(Escadena(xdata)) = "6" Then
     Call DLLGENERALAYUDA.ListarEnTDBGRID(vcon, xtabla, DGrid1, xCampos, xOrden, nlongi, "ISNULL(adescri2,'') like '" & xdato & "%'")
     Text1 = Escadena(xdato): Text1.SelStart = Len(Trim(Text1))
  End If
  ConfigGrid xtipo
End Sub
...
Public Function ConfigGrid(xtipo As Integer)
   ...
   Select Case xtipo
     Case 1
        DGrid1.Columns(0).Width = 1800
        DGrid1.Columns(1).Width = 4900
        If DGrid1.Columns.count = 3 Then
           DGrid1.Columns(2).Width = 1000
        End If
     Case 2
        DGrid1.Columns(0).Width = 1900
