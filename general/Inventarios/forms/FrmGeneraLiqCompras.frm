...
Begin VB.Form FrmGeneraLiqCompras 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(0)   =   "FrmGeneraLiqCompras.frx":0000
      Tab(0).ControlEnabled=   0   'False
      Tab(0).Control(0)=   "CmdGrabardata"
      Tab(0).Control(1)=   "Fr2(0)"
      Tab(0).Control(2)=   "Fr1"
      Tab(0).ControlCount=   3
      TabCaption(1)   =   "Detalle de Guias"
      TabPicture(1)   =   "FrmGeneraLiqCompras.frx":001C
      Tab(1).ControlEnabled=   0   'False
      Tab(1).Control(0)=   "Frame6"
      Tab(1).Control(1)=   "Frame5(1)"
      Tab(1).ControlCount=   2
      TabCaption(2)   =   "Totales de Guias"
      TabPicture(2)   =   "FrmGeneraLiqCompras.frx":0038
      ...
      Begin VB.Frame Frame5 
         ...
         Begin VB.Frame Frame4 
            ...
            Begin VB.CommandButton cmdBotones 
               ...
               Index           =   11
               Left            =   90
               Picture         =   "FrmGeneraLiqCompras.frx":0054
               Style           =   1  'Graphical
               TabIndex        =   21
               Top             =   180
               Width           =   870
            ...
            Begin VB.CommandButton cmdBotones 
               ...
               Index           =   12
               Left            =   1140
               Picture         =   "FrmGeneraLiqCompras.frx":0496
               Style           =   1  'Graphical
               TabIndex        =   20
               Top             =   180
               Width           =   825
      ...
      Begin VB.Frame Frame5 
         ...
         Begin VB.Frame Frame2 
            ...
            Begin VB.CommandButton cmdSalirFinal 
               ...
               Index           =   0
               Left            =   1140
               Picture         =   "FrmGeneraLiqCompras.frx":08D8
               Style           =   1  'Graphical
               TabIndex        =   7
               Top             =   180
               Width           =   825
            ...
            Begin VB.CommandButton cmdGrabaFinal 
               ...
               Height          =   690
               Left            =   90
               Picture         =   "FrmGeneraLiqCompras.frx":0D1A
               Style           =   1  'Graphical
               TabIndex        =   6
               Top             =   180
               Width           =   870
   ...
   End
End
Attribute VB_Name = "FrmGeneraLiqCompras"
...
Private Sub cmdGrabaFinal_Click()
    ...
    If MsgBox("Desea Grabar Las Guias?", vbYesNo, MsgTitle) = vbYes Then
        ...
        If ExisteElem(0, VGCNx, "al_jtempo") Then
           VGCNx.Execute ("delete from al_jtempo")
         Else
            MsgBox "No existe la Tabla Temporal al_jtempo...Verifique!!!", vbInformation, MsgTitle
            Exit Sub
        End If
        
        If ExisteElem(0, VGCNx, "al_jdetatempo") Then
            VGCNx.Execute "delete from al_jdetatempo"
          Else
            MsgBox "No existe la Tabla Temporal al_jdetatempo...Verifique!!!", vbInformation, MsgTitle
            Exit Sub
        End If
       

       nsql = " insert Into al_jtempo ( pedidonumero,puntovtacodigo, clienteruc, clientecodigo, clienterazonsocial,pedidofecha,pedidomoneda )"
       nsql = nsql & "  values('1','" & g_ptoventa & "','" & Trim(csql!CARUC) & "' ,'" & Trim(csql!CACODPRO) & "','" & Trim(csql!CANOMpro) & "','" & csql!CAFECDOC & "','01') "
       VGCNx.Execute (nsql)
       
       nsql = "select * from vt_puntovtadocumento where puntovtacodigo='" & g_ptoventa & "' and documentocodigo='PL'"
       ...
       nume = nrs!puntovtadoccorr
        ...
        nume = Right("000000000000" & RTrim(g_pedserie) + RTrim(nume), 11)
        ...
        nsql = "Update al_jtempo Set pedidonumero='" & nume & "',"
                ...
                If Text1(1).text > 0 Then
                 ...
                 Else
                   If rsdeta!precio > 0 Then
                      ...
                      If rsdeta!moneda = "02" Then precio = rsdeta!precio * IIf(rsdeta!TipoDeCambio = 0, 1, rsdeta!TipoDeCambio)
                    Else
                      precio = 1
                    End If
                End If
                nsql = " Insert Into al_jdetatempo (pedidonumero,detpeditem,detpedcantpedida,productocodigo ,unidadcodigo,detpedmontoprecvta,detpedpreciopact ) "
        ...
        precio = -1
        nsql = " Insert Into al_jdetatempo (pedidonumero,detpeditem,detpedcantpedida,productocodigo ,unidadcodigo,detpedmontoprecvta,detpedpreciopact ) "
        nsql = nsql & " Values('1', " & J & " ," & precio & ",'" & VGCodigo & "','' ," & VGparametros.VGporcentajeimpto & ",0)"
        VGCNx.Execute nsql
        VGCNx.Execute "Update al_jdetatempo " & _
             " Set pedidonumero='" & nume & "'"
        rsdeta.Close
        nrs.Close
                
        VGCNx.BeginTrans
        VGCNx.Execute "insert into al_tempoliquidacionCompra" & g_ptoventa & "  Select * from al_jtempo"
        
        Set nrb = VGCNx.Execute("select * from al_jdetatempo")
        If nrb.RecordCount > 0 Then
            nrs.Open "al_tempodetalleliquidacionCompra" & g_ptoventa, VGCNx, adOpenDynamic, adLockOptimistic
            nrb.MoveFirst
            Do Until nrb.EOF
                nrs.AddNew
        ...
        nrb.MoveFirst
        Do Until nrb.EOF()
           nsql = " Update movalmcab set canroped ='" & nume & "',catipotransf=carftdoc ,"
           nsql = nsql & "canrotransf=carfndoc,casitua=1  where "
...
Private Sub inicializaarchivo()
   ...
   xsql = xsql & " almacencodigo nvarchar (2),fecha datetime )  "
    ...
    Call VGDllGeneral.ListarEnTDBGRID(VGCNx, vt_tempo, TDBGrid2, "vt_tipdoc ,vt_numdoc, clientecodigo ,clienterazonsocial ,documentoreferencia, numeroreferencia , almacencodigo", "almacencodigo", nLongicampo, "")
...
'   xsql = xsql & " almacencodigo nvarchar (2) )  "
   VGCNx.Execute (xsql)
    ...
    Call VGDllGeneral.ListarEnTDBGRID(VGCNx, vt_tempo1, TDBGrid4, "productocodigo, productodescripcion, productocantidad, precio,almacencodigo,moneda,tipocambio", "productocodigo", nLongicampo, "")
    Configuradocumento
End Sub
...
Public Function Listado()
  ...
  Call VGDllGeneral.ListarEnTDBGRID(VGCNx, "movalmcab", TDBGrid1, "carftdoc,carfndoc,caalma,CATD,CANUMDOC, CAFECDOC,CACODCLI,CARUC, CANOMCLI", "cafecdoc", nLongicampo, " catd='NI' and catipmov='I' and carftdoc in('NC') and isnull(canroped,0)=0")
  Call VGDllGeneral.ListarEnTDBGRID(VGCNx, vt_tempo, TDBGrid2, "vt_tipdoc ,vt_numdoc, clientecodigo ,clienterazonsocial ,documentoreferencia, numeroreferencia , almacencodigo", "almacencodigo", nLongicampo, "")
  Call VGDllGeneral.ListarEnTDBGRID(VGCNx, vt_tempo1, TDBGrid4, "productocodigo, productodescripcion, productocantidad, precio,almacencodigo", "productocodigo", nLongicampo, "")
  TDBGrid2.Refresh
  TDBGrid4.Refresh
...
'    nvalor = ""
    ...
    If csql.RecordCount > 0 Then
        ...
        Set acliente = VGCNx.Execute("select * from cp_proveedor where clientecodigo='" & Escadena(csql!CACODPRO) & "'")
    ...
    Do Until csqld.EOF
       ...
       rsdeta.Fields(5) = csqld!DECODMON
