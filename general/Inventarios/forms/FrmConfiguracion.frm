...
Begin VB.Form FrmConfiguracion 
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.TextBox txtcuentahaber 
         ...
         Width           =   1692
      End
      Begin VB.TextBox Textsubasiento 
         Height          =   285
         Left            =   2232
         MaxLength       =   2
         TabIndex        =   9
         ...
         Width           =   864
      End
      Begin VB.CheckBox Chksubasiento 
...
On Error GoTo Err
     If Trim(Textsubasiento.text) = "" And Chksubasiento.Value = 1 Then
                MsgBox "Ingrese el subdiario", vbInformation, "Aviso"
                Textsubasiento.SetFocus
    End If
    rsql = "select * from al_sistema"
    Set adoreg = New ADODB.Recordset
    adoreg.Open rsql, VGCNx, adOpenDynamic, adLockOptimistic
    If ChkAuto.Value = 1 And Chksubasiento.Value = 1 Then
...
On Local Error GoTo ERRAR
    ...
    If Not adoreg.EOF Then
       Textasiento = IIf(IsNull(adoreg("codigoasiento")), "", adoreg("codigoasiento"))
       Chkdiario = IIf(Textasiento <> "", 1, 0)
       Textasiento.Enabled = IIf(Chkdiario.Value = 1, True, False)
       Textsubasiento = IIf(IsNull(adoreg("textsubasiento")), "", adoreg("codigosubasiento"))
       Chksubasiento.Value = IIf(Textsubasiento <> "", 1, 0)
       Textsubasiento.Enabled = IIf(Chksubasiento.Value = 1, True, False)
       txtcuentadebe = cNull(adoreg!cosven_debe)
       txtcuentahaber = cNull(adoreg!cosven_habe)
