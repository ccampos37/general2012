...
Begin VB.Form frmAprobacionRequerimientos 
   AutoRedraw      =   -1  'True
   ...
   ControlBox      =   0   'False
   Icon            =   "frmaprobacionRequerimientos.frx":0000
   LinkTopic       =   "Form1"
   MaxButton       =   0   'False
   MinButton       =   0   'False
   ScaleHeight     =   5700
   ...
   Begin VB.CommandButton CmdSalir 
      ...
      Height          =   675
      Left            =   10440
      Picture         =   "frmaprobacionRequerimientos.frx":08CA
      Style           =   1  'Graphical
      TabIndex        =   3
      Top             =   1200
      Width           =   775
   ...
   Begin VB.CommandButton cmdGra 
      ...
      Height          =   675
      Left            =   10440
      Picture         =   "frmaprobacionRequerimientos.frx":0D0C
      Style           =   1  'Graphical
      TabIndex        =   2
      Top             =   240
      Width           =   775
   ...
   End
End
Attribute VB_Name = "frmAprobacionRequerimientos"
      ...
      Do Until rsdeta2.EOF
         If rsdeta2.Fields(6) > 0 Then
            If VGtipoAprobacion = 1 Then
               SQLd = "UPDATE co_detordcompra SET oc_ncanten=0,"
               SQLd = SQLd & "oc_nsaldo =" & rsdeta2.Fields(6) & ","
               If Val(rsdeta2.Fields(6)) >= Val(rsdeta2.Fields(5)) Then
                  SQLd = SQLd & "estadooccodigo='" & Format(4, "0") & "'"
                ElseIf Val(rsdeta2.Fields(6)) < Val(rsdeta2.Fields(5)) Then
                       SQLd = SQLd & "estadooccodigo='" & Format(3, "0") & "'"
               End If
            Else
               SQLd = "UPDATE co_detordcompra SET oc_ndescto=" & rsdeta2.Fields(6) & ","
               SQLd = SQLd & "oc_nsaldo =" & rsdeta2.Fields(6) & ","
               If Val(rsdeta2.Fields(6)) >= Val(rsdeta2.Fields(5)) Then
                  SQLd = SQLd & "estadooccodigo='" & Format(2, "0") & "'"
                ElseIf Val(rsdeta2.Fields(6)) < Val(rsdeta2.Fields(5)) Then
                       SQLd = SQLd & "estadooccodigo='" & Format(1, "0") & "'"
               End If
            End If
...
Private Sub txtNum_DblClick()
    ...
    SQL = "SELECT a.tipoordencodigo,a.oc_cnumord, a.oc_dfecdoc, b.estadoocdescripcion " & _
             ...
             " where a.oc_estadoorden<>1 "
    If VGOrdenes = 1 Then
       If VGtipoAprobacion = 0 Then
          SQL = SQL & " and c.flagrequerimientosordenes=1 and b.nivelrequerimientoordenes=1  "
        Else
          SQL = SQL & " and c.flagrequerimientosordenes=1  and b.nivelrequerimientoordenes=2 and aprobaciongerencia= 1   "
       End If
    End If
...
Private Sub txtNum_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    If KeyCode = 13 Then txtnum_KeyPress (13)
End Sub
...
Private Sub txtnum_KeyPress(KeyAscii As Integer)
    If KeyAscii = 13 Then
        If txtNum <> "" Then
            ...
            If Not Existe(1, vgtipo + txtNum, "co_cabordcompra", "tipoordencodigo+oc_cnumord", False) Then
...
SQL = "SELECT oc_citem,oc_ccodigo,oc_cdesref=case when oc_ccodigo<>'00' then rtrim(oc_cdesref)+' - '+rtrim(oc_ccomen1) else oc_ccomen1 end,ord_fabnum,oc_ncantid,oc_nsaldo,oc_nsaldo as oc_ncanten " & _
      " FROM co_detordcompra WHERE tipoordencodigo='" & vgtipo
SQL = SQL & "' and oc_cnumord='" & txtNum & "'"
SQL = SQL & " ORDER BY oc_citem"
...
If rbusca1.RecordCount > 0 Then
   ...
   Do Until rbusca1.EOF
      ...
      rsdeta2.Fields(2) = Escadena(Left(rbusca1!OC_CDESREF, 60))
...
Call rsdeta2.Fields.Append("Descripcion", adVarChar, 60)
...
Function Estado_Valido() As Boolean
    ...
    vest = Devolver_Dato(1, vgtipo + txtNum, "co_cabordcompra", "tipoordencodigo+oc_cnumord", False, "estadooccodigo")
    Estado_Valido = False
    If VGtipoAprobacion = 0 Then
       If vest = "0" Then Estado_Valido = True
     Else
...
Sub Muestra_datos_de_co_cabordcompra()
    ...
    SQL = SQL & " on a.oc_ccodpro=b.clientecodigo WHERE tipoordencodigo='" & vgtipo
    SQL = SQL & "' and  oc_cnumord='" & txtNum & "'"
