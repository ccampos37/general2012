...
Begin VB.Form FrmColorAvio 
   ...
   Begin VB.Frame Frame5 
      ...
      Begin VB.ComboBox CmbOrden 
         Height          =   315
         ItemData        =   "FrmColorAvio.frx":0000
         Left            =   5040
         List            =   "FrmColorAvio.frx":000A
   ...
   End
End
Attribute VB_Name = "FrmColorAvio"
...
cNomRepor = "ColorAvio.RPT"
If Trim(cNomRepor) <> "" Then
    CrystalReport1.Reset
    CrystalReport1.WindowTitle = "Reporte de Color de Avios"
...
'If rs.RecordCount > 0 Then
    If Trim(TxFiltro) <> "" Then
        If CmbOrden.ListIndex = 0 Then
            nstr = "CodCol like '" & Trim(UCase(TxFiltro)) & "%'"
        ElseIf CmbOrden.ListIndex = 1 Then
            nstr = "descripcion like '" & Trim(UCase(TxFiltro)) & "%'"
        End If
        nstr = "select * from Color_A where " & nstr
    Else
       nstr = "select * from Color_A"
    End If
'Else
'    nstr = "select * from Color"
'End If
Call Listado(nstr)
End Sub
...
Select Case nCom
Case 0
    Data1.RecordSource = "Select * from Color_A order by CodCol"
    
Case 1
    Data1.RecordSource = "Select * from Color_A order by DESCRIPCIOn"
End Select
TxFiltro = ""
Data1.Refresh
...
Private Sub Command1_Click()
   ...
   Frame3.Caption = "Ingreso de Color de Avios"
...
If rs.RecordCount > 0 Then
   Limpiar
    resp = "N"
    Frame3.Caption = "Modificación de Color de Avios"
    DBGrid1.Visible = False
    'Command19.Visible = False
    Frame2.Visible = False
    Frame5.Visible = False
...
If rs.RecordCount > 0 Then
    ...
    If MsgBox("Seguro de Eliminar", vbQuestion + vbOKCancel, "Inventarios") = vbOK Then
        'If Existe(1, rs.Fields("CODIGO"), "MaeArt", "AColor", False) Then
        '    MsgBox "El Titulo de Tela no puede Eliminarse, porque esta registrada en Articulos"
        'Else
            nTra = 2
            cCodigo1 = Pos_Dato1(rs, "CodCol")
            nTra = 1
            nTra = 0
            
            Call Listado("select * from Color_A")
        'End If
    End If
    DBGrid1.Refresh
...
If resp = "S" Then
  If Trim(Text1) = "" Then
     MsgBox "Ingrese Código de Color de Avios", vbInformation, "Mensaje"
     Text1.SetFocus
     Exit Sub
  Else
       If VGDllGeneral.VerificaDatoExistente(VGCNx, "select * from Color_A where CodCol='" & Trim(Text1) & "'") = 1 Then
          MsgBox "El código de Color de Avios ya existe", vbInformation, "Mensaje"
          Text1.SetFocus
          Exit Sub
       End If
  End If
End If
  ...
  If Trim(Text2) = "" Then
     MsgBox "Ingrese Descripción de Color de Avios", vbExclamation, "Aviso"
     Text2.SetFocus
     Exit Sub
  End If
...
'  End If
    If resp = "S" Then
        VGCNx.Execute "Insert Into Color_A " & _
                          "(CodCol,descripcion)" & _
                          " VALUES(" & _
                          "'" & Trim(Text1.text) & "'," & _
        ...
        Text1.SetFocus
    Else
        VGCNx.Execute "UPDATE Color_A " & _
                          " SET descripcion='" & SupCadSQL(Text2.text) & "'" & _
                          " WHERE CodCol='" & Text1.text & "'"
...
central Me
Call Listado("Select * from Color_A order by CodCol")
'Command19.Visible = True
End Sub
...
If KeyAscii = 13 Then
    If Trim(Text1) <> "" Then
       If Existe(1, Trim(Text1), "Color_A", "CodCol", False) Then
          MsgBox "El código de Color de Avios ya existe", vbInformation, "Mensaje"
...
If KeyAscii = 13 Then
    If Trim(Text2) = "" Then
       MsgBox "Ingrese Descripcion de la Color de Avios ", vbInformation, "Mensaje"
