...
set @FechaActualizacion ='01/12/2006'
set @UsuarioNombre= 'CARLOS'
...
@FechaActualizacion,@UsuarioNombre,'MN','V',0,@NroPase, 
RTRIM(LTRIM(mvtPrvCod)), RTRIM(LTRIM(mvtNPrvNom))
FROM x_ingresos 
...
DEDESCRI,DECANREF1,deordfab,DETR,DETIPCAM) 
select DISTINCT
RTRIM(Almacen) AS ALMACEN,'NI',0,LTRIM(STR(ITEM)) AS ITEM,
LTRIM(STR(Articulo)) AS CODIGO,tot_unidades, mvtPrcUnit,'MN',
' ',0,RTRIM(Serie)+'-'+RTRIM(Numero),@NroPase,0
from x_ingresos 
WHERE NOT ISNULL(ITEM,0)=0
...
LTRIM(STR(Articulo)) AS CODIGO,tot_unidades,0,'MN',
...
--select articulo,count(*) from xx_detalle
--group by articulo having count(*) > 1
