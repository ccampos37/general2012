...
Begin VB.Form FrmGuiaSal 
   ...
   Begin MSFlexGridLib.MSFlexGrid MSFlexGrid1 
      ...
      Rows            =   1
      Cols            =   7
      FormatString    =   $"FormGuiaSal.frx":0000
...
With oCrystalReport
        .Reset
        .ReportFileName = VGParamSistem.RutaReport & "vt_guiaimpresa_" & VGParamSistem.BDEmpresa & VGparametros.empresacodigo & ".rpt"
...
If Rs2(0) <> "3" Then
    Set rs = VGCNx.Execute("select pedidonumero,pedidonrofact,productocodigo,ADESCRI," _
    & " Saldo=(detpedcantpedida)-sum(isnull(decantid,0)),PEDIDOTOTNETO=0,VENDEDORCODIGO," _
    & " almacencodigo , transportecodigo,pedidoentrega,pedidoobserva " _
    & " from " & VGParamSistem.BDEmpresa & ".dbo.v_almacenyventas  " _
    & " WHERE clienteruc='" & ruc & "' and empresacodigo='" & VGparametros.empresacodigo & "' and puntovtacodigo='" & VGparametros.puntovta & "' and pedidonumero='" & Rs2(3) & "' " _
    & " group by pedidonumero,pedidonrofact,productocodigo,ADESCRI,DETPEDCANTPEDIDA,VENDEDORCODIGO," _
    & " almacencodigo ,transportecodigo,pedidoentrega,pedidoobserva having (detpedcantpedida)-sum(isnull(decantid,0))>0 ")
Else
...
If RsCliente.RecordCount = 0 Then
    ...
    & " from " & VGParamSistem.BDEmpresa & ".dbo.v_almacenyventas WHERE CLIENTEruc='" & ruc & "' and empresacodigo='" & VGparametros.empresacodigo & "' and puntovtacodigo='" & VGparametros.puntovta & "' and Estado=0" _
...
'      Text8.Enabled = False
   ...
   If rs("TT_EQUIP") = "S" Then
      ...
      FrmCreacionSal.lblMaq.Visible = True
      FrmCreacionSal.txEquip.Visible = True
   Else
      tx_codmaq.Visible = True  ' False
      Label26.Visible = True    'False
      FrmCreacionSal.lblMaq.Visible = False
      FrmCreacionSal.txEquip.Visible = False
   End If
