      ...
      Select Case SSTab1.Tab
       ...
       Select Case SSTab1.Tab
         Case 0
          ...
          Case 1
                FrmAyudaPedidos.BOrden = "pedidonrofact"
                FrmAyudaPedidos.BCondi = "pedidotipofac='" & adll.ComboDato(Combo3.text) & "' and pedidocondicionfactura='1'"
          Case Else
...
Private Sub aBusca_KeyPress(Index As Integer, KeyAscii As Integer)
  If KeyAscii = 13 Then
     If IsNumeric(aBusca(Index)) Then
        aBusca(Index) = Right("0000000000" & Trim(aBusca(Index)), aBusca(Index).MaxLength)
  End If
...
Private Sub cBusca_Click()
    ...
    SQL = SQL & " and pedidotipofac='" & adll.ComboDato(Combo2.text) & "'and pedidonrofact='" & aBusca(0) & aBusca(1) & "'"
    If VgModificar = 0 Then SQL = SQL & "and puntovtacodigo='" & VGParametros.puntovta & "'"
...
Private Sub cBusca3_Click()
    ...
    If Len(Trim(aBusca(4).text)) > 0 And Len(Trim(aBusca(5).text)) > 0 Then
        If adll.ComboDato(Combo5.text) = g_tipoped Then
           ...
           If VgModificar = 0 Then nsql = nsql & "and puntovtacodigo='" & VGParametros.puntovta & "'"
        Else
           Exit Sub
        End If
        Set rs = VGCNx.Execute(nsql)
...
Public Sub CargaOpcion()
   ...
   Call CargarTipoVentas(Combo1, 1)
   
   Call CargarTipoVentas(Combo2, 2)
   
   Call CargarTipoVentas(Combo3, 2)

   Call CargarTipoVentas(Combo4, 1)

   Combo5.Clear
   Combo5.AddItem g_tipoped & "-PEDIDO"
   Combo5.ListIndex = 0

   Call CargarTipoVentas(Combo6, 1)
...
Private Sub MBox_KeyDown(Index As Integer, KeyCode As Integer, Shift As Integer)
  ...
  ElseIf KeyCode = 112 Then
   If Index = 4 Or Index = 12 Or Index = 20 Then
       ...
       FrmAyudaPedidos.BConexion = VGCNx
       FrmAyudaPedidos.BTabla = "vt_cliente"
       FrmAyudaPedidos.BCampos = "Clientecodigo as Codigo,Clienterazonsocial as Descripcion,Clienteruc as Ruc"
       FrmAyudaPedidos.BOrden = "clienterazonsocial"
       FrmAyudaPedidos.BCondi = ""
       FrmAyudaPedidos.BFiltro = sfiltra
       FrmAyudaPedidos.Show 1
       If Index = 4 Then
   ...
   ElseIf Index = 6 Or Index = 14 Or Index = 22 Then
       ...
       FrmAyudaPedidos.TipoForma = 1
       FrmAyudaPedidos.BConexion = VGCNx
       FrmAyudaPedidos.BTabla = "vt_vendedor"
       FrmAyudaPedidos.BCampos = "vendedorcodigo as Codigo,vendedornombres as Descripcion"
       FrmAyudaPedidos.BOrden = "vendedorcodigo"
       FrmAyudaPedidos.BCondi = ""
