...
Begin VB.Form FrmServicio 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(0)   =   "FrmServicio.frx":0000
      ...
      TabPicture(1)   =   "FrmServicio.frx":001C
      ...
      TabPicture(2)   =   "FrmServicio.frx":0038
      ...
      Begin TabDlg.SSTab SSTab2 
         ...
         TabPicture(0)   =   "FrmServicio.frx":159E
         Tab(0).ControlEnabled=   0   'False
         Tab(0).Control(0)=   "Fr1(0)"
         Tab(0).ControlCount=   1
         TabCaption(1)   =   "Datos Detalle"
         TabPicture(1)   =   "FrmServicio.frx":15BA
         Tab(1).ControlEnabled=   -1  'True
         ...
         TabPicture(2)   =   "FrmServicio.frx":15D6
         Tab(2).ControlEnabled=   0   'False
      ...
      Begin TrueOleDBGrid70.TDBGrid TDBGrid1 
         ...
         _StyleDefs(6)   =   "Style:id=1,.parent=0,.namedParent=33,.bgcolor=&HFFFFFF&,.bold=0,.fontsize=780"
...
Private Sub cAyuda_Click(Index As Integer)
  ...
  ElseIf Index = 3 Then                             ' Ayuda de Productos
       ...
       If Combo2.ListCount > 0 Then
             FrmAyudaPedidos.BTabla = "[" & VGCNx.DefaultDatabase & "].dbo.maeart left join [" & _
...
Private Sub cBoton_Click(Index As Integer)
  ...
  If Index = 0 Then
       ...
       MBox(1) = g_pedserie & Right("000000000000" & TraeDataSerie("select puntovtadoccorr from vt_puntovtadocumento where documentocodigo='" & g_tipoped & "' and puntovtadocserie='" & g_pedserie & "' and puntovtacodigo='" & g_ptoventa & "'", VGCNx), 8)
       MBox(2) = g_facserie & Right("000000000000" & TraeDataSerie("select puntovtadoccorr from vt_puntovtadocumento where documentocodigo='" & g_tipofac & "' and puntovtadocserie='" & g_facserie & "' and puntovtacodigo='" & g_ptoventa & "'", VGCNx), 8)
       MBox(3) = g_bolserie & Right("000000000000" & TraeDataSerie("select puntovtadoccorr from vt_puntovtadocumento where documentocodigo='" & g_tipobol & "' and puntovtadocserie='" & g_bolserie & "' and puntovtacodigo='" & g_ptoventa & "'", VGCNx), 8)
       MBox(4) = g_guiaserie & Right("000000000000" & TraeDataSerie("select puntovtadoccorr from vt_puntovtadocumento where documentocodigo='" & g_tipoguia & "' and puntovtadocserie='" & g_guiaserie & "' and puntovtacodigo='" & g_ptoventa & "'", VGCNx), 8)
...
'    VGcnx.BeginTrans
    ...
    If cOpc(1).Value Or cOpc3(1).Value Then
        If cOpc2(0).Value Then
          ...
          MBox(2) = g_facserie & Right("000000000000" & TraeDataSerie("select puntovtadoccorr from vt_puntovtadocumento where documentocodigo='" & g_tipofac & "' and puntovtadocserie='" & g_facserie & "' and puntovtacodigo='" & g_ptoventa & "'", VGCNx), 8) ' MBox(2).MaxLength)
          MBox(3) = "0": MBox(4) = "0"
          
          If dllgeneral.VerificaDatoExistente(VGCNx, "select * from vt_pedido where pedidonrofact='" & MBox(2) & "' and pedidotipofac='" & g_tipofac & "'") = 1 Then
   ...
   '*Grabar en los cargos ***ctacte ***
    ...
    If (cOpc2(0).Value Or cOpc2(1).Value Or cOpc2(2).Value) And modoventa.ctacte = "1" Then
        ...
        ltipo = "1"
        If dllgeneral.VerificaDatoExistente(VGCNx, "select * from vt_cargo where documentocargo='" & IIf(cOpc2(0).Value, g_tipofac, g_tipobol) & "' and cargonumdoc='" & IIf(cOpc2(0).Value, MBox(3), MBox(4)) & "'") = 0 Then
          ltipo = "1"
        Else
          ltipo = "2"
        End If
...
Private Sub imprimirfacturas()
 ...
 Dim reporte As String
      If cOpc2(0).Value Then
       ...
       End If
       Param(0) = VGCNx.DefaultDatabase
       reporte = "repfactuimpresa_" & VGCNx.DefaultDatabase & ".rpt"
