   ...
   If rori.RecordCount > 0 Then
      ...
      Do Until rori.EOF
         nsql = "INSERT INTO " & sqlfin & _
                ...
                "productoprecvtaofi,productoprecvtareparto,monedacodigo,unidadcodigo,unidadreferencial)" & _
                " VALUES (" & _
                ...
                IIf(IsNull(rori!productoprecvta), CDbl(0), rori!productoprecvta) * (1 + (sdreparto / 100)) & "," & _
                 "'" & Escadena(rori!monedacodigo) & "','" & Escadena(rori!unidadcodigo) & "','" & Escadena(rori!unidadreferencial) & "')"
         VGCNx.Execute nsql
...
Public Sub ProcesadataMaeart()
   ...
   If rori.RecordCount > 0 Then
      ...
      rori.MoveFirst
      Do Until rori.EOF
         SQL = "INSERT INTO " & sqlfin & "(productocodigo,productodescripcion,productoprecvta,"
         SQL = SQL & "productodescrcorta, productofamiliacodigo,productoporcimpto,"
         SQL = SQL & "productoestunidreferencia,unidadfactorconv,monedacodigo,unidadcodigo)"
         SQL = SQL & " VALUES ('" & rori!acodigo & "','" & rori!ADESCRI & "',0,"
         SQL = SQL & "'','" & rori!afamilia & "'," & sdventa & ","
         SQL = SQL & "'',1,'01','" & rori!aunidad & "')"
         VGCNx.Execute SQL
         Bar1.Value = Bar1.Value + 1
         DoEvents
         rori.MoveNext
...
'      End If
        ...
        nsql = "UPDATE " & sqlfin & _
                ...
                " Where productocodigo='" & RTrim(FrmListaPrecios.TDBGrid2.Columns(0).text) & "'"
...
Public Property Let Btipo(pdata As Integer)
   stipo = pdata
End Property
Public Property Let Bmodo(pdata As Integer)
   smodo = pdata
End Property
