...
Begin VB.Form FrmUnidadMedida 
   Caption         =   "Mantenimiento de Unidad de Medida"
   ClientHeight    =   7395
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   9855
   ...
   Begin VB.Frame frmbotones 
      ...
      Begin VB.CommandButton cmdBotones 
         ...
         Index           =   3
         Left            =   3510
         Picture         =   "FrmUnidadMedida.frx":0000
         Style           =   1  'Graphical
         TabIndex        =   13
         Top             =   180
         Width           =   870
      ...
      Begin VB.CommandButton cmdBotones 
         ...
         Index           =   4
         Left            =   4590
         Picture         =   "FrmUnidadMedida.frx":0442
         Style           =   1  'Graphical
         TabIndex        =   12
         Top             =   180
         Width           =   915
      ...
      Begin VB.CommandButton cmdBotones 
         ...
         Index           =   2
         Left            =   2440
         Picture         =   "FrmUnidadMedida.frx":0884
         Style           =   1  'Graphical
         TabIndex        =   11
         Top             =   180
         Width           =   870
      ...
      Begin VB.CommandButton cmdBotones 
         ...
         Index           =   1
         Left            =   1320
         Picture         =   "FrmUnidadMedida.frx":0CC6
         Style           =   1  'Graphical
         TabIndex        =   10
         Top             =   180
         Width           =   915
      ...
      Begin VB.CommandButton cmdBotones 
         ...
         Index           =   0
         Left            =   225
         Picture         =   "FrmUnidadMedida.frx":1108
         Style           =   1  'Graphical
         TabIndex        =   9
         Top             =   180
         Width           =   915
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      TabPicture(0)   =   "FrmUnidadMedida.frx":154A
      Tab(0).ControlEnabled=   -1  'True
      Tab(0).Control(0)=   "TDBGridProducto"
      Tab(0).Control(0).Enabled=   0   'False
      Tab(0).ControlCount=   1
      TabCaption(1)   =   "Mantenimiento"
      TabPicture(1)   =   "FrmUnidadMedida.frx":1566
...
Private Sub cAcepta_Click()
   ...
   Dim s_codigounidadref As String
   Dim d_factorconv As Double
   
   On Error GoTo CONTROLERRORES
   ''''''''
     
     If cmbRef.ListIndex <> -1 And txt(3) = "" Then
            MsgBox "Ingrese Factor de Conversión", vbCritical, "Error"
            cAcepta.Enabled = False
            Exit Sub
     End If
       
     If cmbRef.ListIndex <> -1 Then
        s_codigounidadref = ArregloRef(0, cmbRef.ListIndex)
     Else
        s_codigounidadref = ""
     End If
   ...
   If modoinsert = True Then
         ...
         End If
          ...
          SQL = "INSERT INTO vt_unidad " & _
               "(unidadcodigo,unidaddescripcion," & _
               "unidaddescrcorta,unidadreferencial," & _
               "unidadfactorconv,estadoreg,usuariocodigo,fechaact " & _
               ") VALUES " & _
               "('" & txt(0) & "','" & txt(1) & "'," & _
               "'" & txt(2) & "','" & s_codigounidadref & "'," & d_factorconv & "," & _
                chk(0).Value & _
               ",'" & g_usuario & "','" & Date & "')"
    ...
    ElseIf modoedit = True Then
            ...
            SQL = "UPDATE vt_unidad SET " & _
               "unidadcodigo='" & txt(0) & "'," & _
               "unidaddescripcion='" & txt(1) & "'," & _
               "unidaddescrcorta='" & txt(2) & "'," & _
               "unidadreferencial='" & s_codigounidadref & "'," & _
               "unidadfactorconv=" & d_factorconv & "," & _
               "estadoreg=" & chk(0).Value & "," & _
               "usuariocodigo='" & g_usuario & "'," & _
               "fechaact='" & Date & "' " & _
               "WHERE unidadcodigo='" & i_codigounidad & "'"
...
Private Sub cmbref_KeyDown(KeyCode As Integer, Shift As Integer)
 If KeyCode = 13 Then SendKeys "{TAB}"
End Sub
...
Private Sub cmdBotones_Click(Index As Integer)
  ...
  Select Case Index
     Case 0   'nuevo
            ...
            Call Cargar_Combo   'Llena Combo Unidad Referencial
     ...
     Case 1   'modificar
         ...
         i_codigounidad = TDBGridProducto.Columns(0).Text
        ...
        If TDBGridProducto.Columns(6).Value = False Then
             chk(0).Value = 0
        ElseIf TDBGridProducto.Columns(6).Value = True Then
            chk(0).Value = 1
        End If
                 
        Call Cargar_Combo(Trim(TDBGridProducto.Columns(0).Text))    'Llena Combo Unidad Referencial
        Call fncSeleccionaCombo(Trim(TDBGridProducto.Columns(3).Text), cmbRef, ArregloRef)
     ...
     End If
       ...
       If MsgBox("Desea eliminar el registro?", vbYesNo + vbDefaultButton2, "AVISO") = vbYes Then
          SQL = "DELETE FROM vt_unidad WHERE unidadcodigo = '" & TDBGridProducto.Columns(0).Text & "'"
          cn.Execute SQL
          Mostrar_Data
       End If
        
     Case 3   'imprimir
         Call Imprimir("RepvtMantUnidad.rpt")
     Case 4  ' salir
       Unload Me
  End Select
Exit Sub
...
Private Sub Form_Load()
 ...
 'oCrystalReport.ReportFileName = RutaRep & "MantPuntoVtaDoc.rpt"
End Sub
...
Public Function Mostrar_Data()
  ...
  Dim I As Integer
      ...
      SQL = "SELECT a.unidadcodigo as Código," & _
      "a.unidaddescripcion as 'Descripción'," & _
      "a.unidaddescrcorta as 'Desc.Corta'," & _
      "a.unidadreferencial as 'Cód.Unid.Ref.'," & _
      "b.unidaddescripcion as 'Desc.Unid.Ref.'," & _
      "a.unidadfactorconv as 'Factor Conv.'," & _
      "a.estadoreg as 'Activo'" & _
      " " & _
      "FROM vt_unidad a " & _
      "     LEFT OUTER JOIN  vt_unidad b " & _
      "     ON a.unidadreferencial = b.unidadcodigo" & _
      "     ORDER BY a.unidadcodigo"
      
      Set rs = cn.Execute(SQL)
      Set TDBGridProducto.DataSource = rs
    
      ' COMBO UNIDAD REF:
      SQL = "SELECT unidadcodigo,unidaddescripcion " & _
      "FROM vt_unidad"
      
      Set rs = cn.Execute(SQL)
      If rs.RecordCount > 0 Then
        ReDim ArregloRef(0 To 1, 0 To rs.RecordCount - 1)
        Call fncLlenarArreglo_Combo(rs, cmbRef, ArregloRef, 1)
      End If
       ...
       For I = 0 To TDBGridProducto.Columns.Count - 1
        Select Case I
            Case 0, 1, 2, 3, 4, 5
                ...
                TDBGridProducto.Columns(I).Width = 1200
            Case 6
...
Validar_CodigosDuplicados = False
  ...
  Do Until TDBGridProducto.EOF
     ...
     ElseIf operacion = "UPDATE" Then
        If Trim(txt(0)) = _
           Trim(TDBGridProducto.Columns.Item(0).Value) _
...
If txt(Index) <> "" Then
    ...
    End If
    If Index = 3 Then
        txt(3).Text = Format(CDbl(txt(3).Text), "##,##0.00")
    End If
End If
End Sub
...
Private Function Cargar_Combo(Optional condicion As String)
...
' COMBO UNIDAD REF:
      where = ""
      If condicion <> "" Then
        where = " WHERE unidadcodigo <> '" & condicion & "'"
      End If
      
      SENsql = _
            "SELECT unidadcodigo,unidaddescripcion " & _
            "FROM vt_unidad" & where & _
            " ORDER BY unidaddescripcion "
      
      Set rs = cn.Execute(SENsql)
...
Validar_DescripcionesDuplicadas = False
   ...
   Do Until TDBGridProducto.EOF
      ...
      ElseIf operacion = "UPDATE" Then
          If Trim(txt(1)) = _
             Trim(TDBGridProducto.Columns.Item(1).Value) _
          And TDBGridProducto.Row <> filaorigen Then
                 Validar_DescripcionesDuplicadas = True
                 Exit Function
          End If
