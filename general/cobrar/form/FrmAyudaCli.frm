...
'FIXIT: Declare 'AOrden' con un tipo de datos de enlace en tiempo de compilación           FixIT90210ae-R1672-R1B8ZE
Dim AOrden, ACondi As String
Dim DLLGENERALAYUDA As New dllgeneral.dll_general
Dim vcon As New ADODB.Connection
'FIXIT: Declare 'xtabla' and 'xCampos' and 'xOrden' con un tipo de datos de enlace en tiempo de compilación     FixIT90210ae-R1672-R1B8ZE
Dim xtabla, xCampos, xOrden, xCondi As String
'FIXIT: Declare 'xdata' con un tipo de datos de enlace en tiempo de compilación            FixIT90210ae-R1672-R1B8ZE
Dim xdata, xdato As String

Dim nlongi(1) As Integer
'FIXIT: Declare 'nfiltra' con un tipo de datos de enlace en tiempo de compilación          FixIT90210ae-R1672-R1B8ZE
Dim nfiltra()
    ...
    If DGrid1.Row >= 0 Then
        Select Case xtipo
          Case 3, 4, 1
            ...
            nDetalle = RTrim$(DGrid1.Columns(1).text)
            If DGrid1.Columns.Count = 5 Then
               nsaldo = ESNULO(DGrid1.Columns(4).text, 0)
            End If
          Case 2
...
Private Sub Form_Load()
  ...
  If RTrim$(Escadena(xdata)) = "1" Then
     Call DLLGENERALAYUDA.ListarEnTDBGRID(vcon, xtabla, DGrid1, xCampos, xOrden, nlongi, "productodescripcion like '" & xdato & "%'")
     Text1 = Escadena(xdato): Text1.SelStart = Len(RTrim$(Text1))
  ElseIf RTrim$(Escadena(xdata)) = "2" Then
     Call DLLGENERALAYUDA.ListarEnTDBGRID(vcon, xtabla, DGrid1, xCampos, xOrden, nlongi, "clienterazonsocial like '" & xdato & "%'")
     Text1 = Escadena(xdato): Text1.SelStart = Len(RTrim$(Text1))
  Else
     Call DLLGENERALAYUDA.ListarEnTDBGRID(VGCNx, xtabla, DGrid1, xCampos, xOrden, nlongi, xCondi)    'vcon
  End If
  ConfigGrid xtipo
End Sub


'FIXIT: Declare 'campos' con un tipo de datos de enlace en tiempo de compilación           FixIT90210ae-R1672-R1B8ZE
   ...
   Select Case xtipo
     Case 1
        ...
        If DGrid1.Columns.Count = 3 Then
           ...
           DGrid1.Columns(2).NumberFormat = "##,###,##0.00"
        End If
     Case 2
...
'FIXIT: Declare 'pdata' con un tipo de datos de enlace en tiempo de compilación            FixIT90210ae-R1672-R1B8ZE
...
Public Property Let BOrden(pdata As String)
   xOrden = IIf(IsNull(pdata), "", RTrim$(pdata))
   AOrden = IIf(IsNull(pdata), "", RTrim$(pdata))
End Property

Public Property Let BCondi(pdata As String)
   xCondi = IIf(IsNull(pdata), "", RTrim$(pdata))
   ACondi = IIf(IsNull(pdata), "", RTrim$(pdata))
