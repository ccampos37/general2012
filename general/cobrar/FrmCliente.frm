...
'FIXIT: Declare 'modoinsert' con un tipo de datos de enlace en tiempo de compilación       FixIT90210ae-R1672-R1B8ZE
...
Private Sub cAyuda_Click(Index As Integer)
    ...
    Else
        MsgBox "Ingrese Código del Cliente", vbCritical, "AVISO"
    End If
End Sub
    ...
    Case 2   'Boton Eliminar
       If TDBGrid1.Row >= 0 Then
          If MsgBox("Desea eliminar el registro?", vbYesNo + vbDefaultButton2, "AVISO") = vbYes Then
             SQL = " select * from vt_cargo where clientecodigo='" & TDBGrid1.Columns(0).Text & "'"
             Set RSQL = VGCNx.Execute(SQL)
             If RSQL.RecordCount > 0 Then
                MsgBox ("existe informacion del cliente , NO SE PUEDE ELIMINAR ")
                Exit Sub
             End If
...
'FIXIT: Declare 'CargaData' con un tipo de datos de enlace en tiempo de compilación        FixIT90210ae-R1672-R1B8ZE
...
Private Sub MBox_KeyPress(Index As Integer, KeyAscii As Integer)
   Dim rsqlValida As New ADODB.Recordset
    cmdBotones(11).Enabled = Validar_DatosNulos()
    cmdBotones(5).Enabled = False
   If KeyAscii = 13 Then
     If Index = 0 Then
        If Not (Len(MBox(Index)) = 11) Then
            MsgBox "Ingrese un Codigo de cliente  válido (11 digitos)..", vbInformation, MsgTitle
            Exit Sub
        If modoinsert = True Then
            SQL = "select * from vt_cliente where clientecodigo='" & MBox(0).Text & "'"
            ...
            MBox(1) = MBox(0)
        ElseIf modoedit = True Then
            SQL = "select * from vt_cliente where clientecodigo ='" & RTrim$(MBox(0).Text) & "'"
            SQL = SQL & " and clientecodigo <> '" & RTrim$(i_codigocliente) & "'"
            If adll.VerificaDatoExistente(VGCNx, SQL) = 1 Then
                MsgBox MsgAdd, vbInformation, MsgTitle
                Exit Sub
            End If
     ...
     ElseIf Index = 1 Then
        If Not (Len(MBox(Index)) = 11) Then
          MsgBox "Ingrese un numero de RUC válido ( 11 Digitos)...", vbInformation, MsgTitle
          Exit Sub
        End If
        SQL = "select * from vt_cliente where clienteruc ='" & RTrim$(MBox(1).Text) & "'"
        SQL = SQL & " and clientecodigo <> '" & RTrim$(MBox(0).Text) & "'"
        Set rsqlValida = VGCNx.Execute(SQL)
        If rsqlValida.RecordCount > 0 Then
                MsgBox ("Codigo de RUC existe, en el codigo --> " & rsqlValida("clientecodigo"))
                Exit Sub
            End If
        End If
...
'FIXIT: Declare 'GrabaCliente' con un tipo de datos de enlace en tiempo de compilación     FixIT90210ae-R1672-R1B8ZE
...
Private Sub fncBusqueda(conexion As Connection, grid As TDBGrid)
    Dim SQL As String
    Dim where As String
