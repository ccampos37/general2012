...
Begin VB.Form FrmXXXNotaFisico 
   ...
   Begin TabDlg.SSTab SSTab1 
      ...
      EndProperty
      TabCaption(0)   =   "                                            "
      TabPicture(0)   =   "FrmXXXNotaFisico.frx":0000
      ...
      Begin VB.Frame Frame3 
         ...
         Begin VB.Image Image1 
            Height          =   480
            Left            =   150
            Picture         =   "FrmXXXNotaFisico.frx":001C
            Top             =   330
            Width           =   480
         End
      ...
      Begin VB.Frame Frame4 
         ...
         Begin VB.CommandButton cmdBotones 
            ...
            Index           =   12
            Left            =   1050
            Picture         =   "FrmXXXNotaFisico.frx":0609
            Style           =   1  'Graphical
            TabIndex        =   4
            Top             =   180
            Width           =   855
         ...
         Begin VB.CommandButton cmdBotones 
            ...
            Index           =   11
            Left            =   90
            Picture         =   "FrmXXXNotaFisico.frx":0A4B
            Style           =   1  'Graphical
            TabIndex        =   3
            Top             =   180
            Width           =   870
   ...
   End
End
Attribute VB_Name = "FrmXXXNotaFisico"
   ...
   Select Case Index
    Case 11
        ...
        If IsNull(MBox1(2).ClipText) Or Len(Trim(MBox1(2).ClipText)) = 0 Or CDbl(MBox1(2).ClipText) <= 0 Then
           ...
           Exit Sub
        End If
        If dllgeneral.VerificaDatoExistente(cn, "select * from vt_cliente where clientecodigo='" & Ctr_Ayuda1.xclave & "' and clientesuspendido='1'") = 1 And Ctr_Ayuda1.xclave <> g_Eventual Then
           MsgBox W1TXT3, vbInformation, MsgTitle
           Exit Sub
        End If
        If dllgeneral.VerificaDatoExistente(cn, "select * from vt_cliente where clientecodigo='" & Ctr_Ayuda1.xclave & "' and ((clientelimitecreddolar-clientesaldodolares)*" & MBox(8) & "+ (clientelimitecredsoles-clientesaldosoles))-" & TNeto & " <=0") = 1 And Ctr_Ayuda1.xclave <> g_Eventual Then
        ...
        If GrabarData() = 1 Then
          cn.CommitTrans
          g_TipoMovi = 0
          If MsgBox("Desea Imprimir la Nota de Credito", vbYesNo, "AVISO") = vbYes Then
              Call ImprimirNota
          End If
...
Dim rb3 As New ADODB.Recordset
Dim cuenta As Integer, j As Integer
Dim rab As New ADODB.Recordset
Dim nloca As Integer, npos As Integer
Dim glosa As String

   Set rab = cn.Execute("select * from vt_documento where documentocodigo='" & MBox(6) & "'")
   If rab.RecordCount > 0 Then
      glosa = "" & Trim(Ctr_Ayuda3.xnombre) & " SEGUN DOCUMENTO " & Left(Trim("" & rab!documentodescrcorta), 3) & "-" & Trim(MBox(7) & MBox(8))
   Else
      glosa = "" & Trim(Ctr_Ayuda3.xnombre) & " SEGUN DOCUMENTO " & MBox(6) & "-" & Trim(MBox(7) & MBox(8))
   End If
   rab.Close
   Set rab = Nothing


    Set rb2 = cn.Execute("select * from vt_pedido where pedidotipofac='" & Trim(dllgeneral.ComboDato(Combo2.Text)) & "' and pedidonrofact='" & Trim(MBox(1)) & Trim(MBox(2)) & "' and pedidocondicionfactura='0'")
    If rb2.RecordCount > 0 Then
        cn.Execute "delete from tempfile"
        
        npos = 1
        cuenta = Len(Trim(glosa))
        For j = 1 To cuenta
              nloca = InStr(j, glosa, Chr(13))
              If nloca > 0 Then
                cn.Execute "INSERT INTO tempfile(detpedcantpedida,productocodigo,productodescripcion,detpedmontoprecvta,detpedimpbruto,detpeddsctoxitem,detpedfactorconv,unidadcodigo)" & _
                           " VALUES ( ' ',' ','" & Mid(glosa, npos, nloca) & "',null,null,null,null,' ')"
                 npos = nloca + 1
                 j = npos
              Else
                cn.Execute "INSERT INTO tempfile(detpedcantpedida,productocodigo,productodescripcion,detpedmontoprecvta,detpedimpbruto,detpeddsctoxitem,detpedfactorconv,unidadcodigo)" & _
                       " VALUES ( ' ',' ','" & Trim(glosa) & "',null,null,null,null,' ')"
                   j = cuenta
              End If
         Next j
        
        cn.Execute "INSERT into tempfile" & _
                 " Select a.detpedcantpedida,a.productocodigo,b.productodescripcion,(a.detpedimpbruto/a.detpedcantpedida),a.detpedimpbruto,a.detpeddsctoxitem,isnull(a.detpedfactorconv,0),'Kgs' " & _
                 " From vt_detallepedido A join " & "listapre" & CStr(Val(rb2!pedidolistaprec)) & " B" & _
                 " On A.productocodigo=B.productocodigo " & _
                 " Where pedidonumero='" & Trim(rb2.Fields("pedidonumero")) & "' and b.almacencodigo='" & Trim(rb2.Fields("almacencodigo")) & "'"
         
         Set rb = cn.Execute("select * from tempfile")
         If rb.RecordCount > 0 Then
          Set rb3 = cn.Execute("select * from vt_modoventa where modovtacodigo='" & Trim(dllgeneral.ComboDato(Combo2.Text)) & "'")
          If rb3.RecordCount > 0 Then
             cuenta = rb3.Fields("modovtaitemxdoc") - rb.RecordCount
          Else
             cuenta = (21 - rb.RecordCount)
          End If
          rb3.Close
          Set rb3 = Nothing
        ...
        oCrystalReport.ReportFileName = RutaRep & "RepNotaimpresa.rpt"
        ...
        oCrystalReport.DiscardSavedData = True
         ...
         With oCrystalReport
               .Formulas(0) = "nro='" & rb2!pedidonrofact & "'"
                Set rb3 = cn.Execute("select * from vt_formapago where formapagocodigo='" & rb2!formapagocodigo & "'")
                If rb3.RecordCount > 0 Then
                  .Formulas(1) = "cpago='" & Escadena(rb2!formapagocodigo) & "-" & Escadena(Trim(rb3!formapagodescripcion)) & "'"
                Else
                   .Formulas(1) = "cpago=''"
                End If
                rb3.Close
                Set rb3 = Nothing
               .Formulas(2) = "opedido='" & rb2!pedidonumero & "'"
               .Formulas(3) = "ocompra=''"
               .Formulas(4) = "guia=''"
               .Formulas(5) = "fechaven='" & CStr(rb2!pedidofechafact) & "'"
               .Formulas(6) = "vendedor='" & Escadena(rb2!vendedorcodigo) & "'"
               .Formulas(7) = "bruto='" & Numero(MBox2(7)) & "'"
               .Formulas(8) = "dscto='" & MBox2(8) & "'"
               .Formulas(9) = "igv='" & MBox2(9) & "'"
               .Formulas(10) = "distrito='" & Escadena(rb2!clientedireccion) & "'"
               .Formulas(11) = "cliente='" & Escadena(rb2!clientecodigo) & "-" & Escadena(rb2.Fields("clienterazonsocial")) & "'"
               .Formulas(12) = "fecha='" & CStr(rb2!pedidofechafact) & "'"
               .Formulas(13) = "direccion='" & Escadena(rb2!clientedireccion) & "'"
               .Formulas(14) = "dni='" & Escadena(rb2!clienteruc) & "'"
               .Formulas(15) = "letras= '" & dllgeneral.NUMLET(Numero(Round(CDbl(MBox2(10)), 2))) & IIf(dllgeneral.ComboDato(Combo1.Text) = g_TipoSol, "Nuevos Soles", "Dolares Americanos") & "'"
               .Formulas(16) = "concepto='" & Trim(Ctr_Ayuda3.xclave) & "-" & Trim(Ctr_Ayuda3.xnombre) & "'"
...
Private Sub Form_Load()
   ...
   Call dllgeneral.llenacombo(Combo2, "select * from cc_tipodocumento inner join cc_parametro on cc_tipodocumento.tdocumentocodigo=cc_parametro.tdocumentonotaabono or cc_tipodocumento.tdocumentocodigo=cc_parametro.tdocumentonotacargo or cc_tipodocumento.tdocumentocodigo=cc_parametro.tdocumentonotaabobo or cc_tipodocumento.tdocumentocodigo=cc_parametro.tdocumentonotacarbo", cn)
...
Public Function GrabarData() As Integer
    ...
    If dllgeneral.VerificaDatoExistente(cn, "select * from vt_modoventa where modovtacodigo='" & Trim(dllgeneral.ComboDato(Combo2.Text)) & "'") = 0 Then
        MsgBox "No existe el modo de venta " & Trim(dllgeneral.ComboDato(Combo2.Text)) & " para generar el documento...Verifique!!", vbInformation, "AVISO"
        Exit Function
    End If
    If CDbl(MBox(4)) <> CDbl(MBox2(10)) Then
       MsgBox "Los Totales no son iguales...Verifique!!!", vbInformation, MsgTitle
    ...
    If asql.RecordCount > 0 Then
       wCabe(2) = g_pedserie & Right("000000000000" & Trim(asql!puntovtadoccorr), 8)
    End If
    asql.Close
    Set asql = Nothing
    ...
    If dllgeneral.VerificaDatoExistente(cn, "select * from cc_tipodocumento where tdocumentocodigo='" & dllgeneral.ComboDato(Combo2.Text) & "' and tdocumentotipo='A'") = 1 Then
      tcargo = "A"
    Else
      tcargo = "C"
    End If
...
'         Set acmd.ActiveConnection = cbdatos
'         acmd.CommandType = adCmdStoredProc
'         acmd.CommandTimeout = 0
