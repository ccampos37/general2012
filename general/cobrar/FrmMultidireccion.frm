...
Begin VB.Form FrmMultidireccion 
   ...
   Caption         =   "Mantenimiento Cliente - Dirección"
   ClientHeight    =   5550
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   9435
...
'FIXIT: Declare 'i_codigocliente' and 'i_razonsocial' con un tipo de datos de enlace en tiempo de compilación     FixIT90210ae-R1672-R1B8ZE
...
Private Sub cAcepta_Click()
   ...
   '''''''
   If i_modoinsert = True Then  ' Mantenimiento  Cliente
        NombreTabla = "Tempo_clientedireccion"
   Else
        NombreTabla = "vt_clientedireccion"
   End If
  ...
  Select Case Index
     ...
     Case 1   'modificar
         ...
         End If
             ...
             txt(0) = i_codigocliente
             txt(1) = i_razonsocial
             txt(2) = RTrim$(TDBGridProducto.Columns(0).Text)
             txt(3) = RTrim$(TDBGridProducto.Columns(1).Text)
             txt(4) = RTrim$(TDBGridProducto.Columns(2).Text)
...
'FIXIT: Declare 'Mostrar_Data' con un tipo de datos de enlace en tiempo de compilación     FixIT90210ae-R1672-R1B8ZE
...
Validar_DatosNulos = False
                ...
                If RTrim$(txt(2)) <> "" And RTrim$(txt(3)) <> "" Then
 ...
 TDBGridProducto.MoveFirst
   Do Until TDBGridProducto.EOF
      If operacion = "INSERT" Then
         If RTrim$(txt(2)) = _
                    RTrim$(TDBGridProducto.Columns(0).Text) Then
      ...
      ElseIf operacion = "UPDATE" Then
         If RTrim$(txt(2)) = _
               RTrim$(TDBGridProducto.Columns(0).Text) _
...
txt(indice) = Right$(CADENA & RTrim$(txt(indice)), txt(indice).MaxLength)
