...
Public Function GrabarData() As Integer
    ...
    If adll.VerificaDatoExistente(VGCNx, "select * from vt_abono where documentoabono='" & RTrim$(Ctr_TipoDoc.xclave) & "' and  abononumdoc='" & RTrim$(Ctr_NumDoc.xclave) & "'") = 0 Then
         VGCNx.Execute " Update vt_cargo " & _
                    " Set cargoapeflgreg='1' " & _
                    " where documentocargo='" & RTrim$(Ctr_TipoDoc.xclave) & "' and cargonumdoc='" & RTrim$(Ctr_NumDoc.xclave) & "'"
         
         SQL = "UPDATE vt_cargo set "
         ...
         SQL = SQL & "from  vt_abono A,"
         SQL = SQL & "(select  A.* from vt_cargo A,cc_tipoplanilla d "
         SQL = SQL & "where A.abonotipoplanilla=d.tplanillacodigo and  d.tplanillacanjes='1' and "
         SQL = SQL & "A.clientecodigo like '" & RTrim$(Ctr_Cliente.xclave) & "' and "
         SQL = SQL & "A.documentocargo='" & RTrim$(Ctr_TipoDoc.xclave) & "' and "
         SQL = SQL & "A.cargonumdoc like '" & RTrim$(Ctr_NumDoc.xclave) & "') as ZZ "
         ...
         rsdeta.MoveFirst
         Do Until rsdeta.EOF
            SQL = "DELETE vt_abono WHERE "
            SQL = SQL & "abononumplanilla like '" & rsdeta("abononumplanilla").Value & "'  and "
            SQL = SQL & "abonocancli='" & RTrim$(Ctr_Cliente.xclave) & "'"
            VGCNx.Execute (SQL)
            rsdeta.MoveNext
         Loop
...
Private Sub Ctr_TipoDoc_AlDevolverDato(ByVal ColecCampos As ADODB.Fields)
  ...
  Ctr_NumDoc.Filtro = "documentocargo='" & ColecCampos(0).Value & "' and isnull(cargoapeflgcan,0)=0 and isnull(cargoapeflgreg,0)<>1 and clientecodigo='" & RTrim$(Ctr_Cliente.xclave) & "'"
  Ctr_NumDoc.Ejecutar
End Sub
...
Function ValidaData() As Boolean
   ...
   If adll.VerificaDatoExistente(VGCNx, "select * from vt_cargo where documentocargo='" & RTrim$(Ctr_TipoDoc.xclave) & "' and cargonumdoc='" & RTrim$(Ctr_NumDoc.xclave) & "' and clientecodigo='" & Ctr_Cliente.xclave & "' and cargoapeflgreg='1'") = 1 Then
    ...
    If IsNull(Ctr_Cliente.xclave) Or RTrim$(Ctr_Cliente.xclave) = Empty Then
...
Sub MuestraData()
  ...
  SQL = SQL & "i.monedasimbolo as MonCargo from  vt_abono A,"
  SQL = SQL & "(select  A.* from vt_cargo A, cc_tipoplanilla d "
  ...
  SQL = SQL & "A.clientecodigo like '" & RTrim$(Ctr_Cliente.xclave) & "' and A.documentocargo='" & RTrim$(Ctr_TipoDoc.xclave) & "' and "
  SQL = SQL & "A.cargonumdoc='" & RTrim$(Ctr_NumDoc.xclave) & "') as ZZ,"
  SQL = SQL & "vt_cliente e,cc_tipodocumento f,"
  SQL = SQL & "gr_moneda g,cc_tipodocumento h,"
  SQL = SQL & "gr_moneda I "
  ...
  If Not rsdeta.BOF Or Not rsdeta.EOF Then
    ...
    NumPlanilla = rsdeta("abononumplanilla").Value
