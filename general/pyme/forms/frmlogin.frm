   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.Frame Frame2 
         ...
         Begin VB.Label Label2 
            ...
            Caption         =   "Inventarios, Ventas y Cobranzas"
            BeginProperty Font 
               Name            =   "Tahoma"
               Size            =   12
               Charset         =   0
      ...
      Begin VB.ComboBox Combo3 
         Height          =   315
         ItemData        =   "frmlogin.frx":3BE51
         Left            =   1380
         List            =   "frmlogin.frx":3BE5B
      ...
      Begin VB.ComboBox Combo4 
         ForeColor       =   &H00800000&
         Height          =   315
         ItemData        =   "frmlogin.frx":3BE77
         Left            =   1410
         List            =   "frmlogin.frx":3BE79
         Style           =   2  'Dropdown List
         TabIndex        =   4
         Top             =   3000
         Width           =   2700
      ...
      Begin VB.Label Label1 
         AutoSize        =   -1  'True
         BackStyle       =   0  'Transparent
         Caption         =   "RPC993900810/990381193/Nextel 41*156*5229"
         BeginProperty Font 
            Name            =   "Tahoma"
            Size            =   8.25
            Charset         =   0
...
If DTPfecha > Date Then
    ...
    Exit Sub
End If
 VGParamSistem.stockcomp = 0
VGParametros.controlaestadosrendicion = False
VGParametros.diasatrazorendicion = 0
...
g_ptoventa = Left(Combo4.text, 2)
g_usuario = Trim(txtUserName)
Call CargarParametrosCompras
Call CargarParametrosVentas
...
Set RSAUX = VGCNx.Execute(" select * from co)al_sistema where empresacodigo='" & Left(Combo2.text, 2) & "'")
...
If RSAUX.RecordCount > 0 Then
   ...
   VGParametros.SaldoConsolidadoxPedidos = RSAUX!SaldoConsolidadoxPedidos
 Else
...
ElseIf Combo3.ListIndex = 1 Then 'Usuario
    vGAdmLog = False
    SQL = "Select * from si_usuario WHERE usuariocodigo='" & VGUsuario & "'"
    Set reg = VGConfig.Execute(SQL)
     Buscar_en_BD = False
     If reg.RecordCount <> 0 Then
     reg.MoveFirst
     Do While Not reg.EOF
        If UCase(RTrim$(txtUserName)) = UCase(reg.Fields("usuariocodigo")) And UCase(RTrim$(txtPassword)) = DECODIFICA(reg.Fields("UsuarioPassword"), NUMMAGICO) Then
            VGUsuario = UCase(txtUserName)
            VGPass = txtPassword
            Buscar_en_BD = True
            Exit Do
...
If Combo1.ListCount > 0 Then
    ...
    If Not rs.EOF Then
      ...
      If Trim(VGParametros.nombreguia) = "" Then VGParametros.nombreguia = "al_guiaremision"
      VGParametros.nombrefactura = ESNULO(rs!nombrefactura, "al_factura")
      If Trim(VGParametros.nombrefactura) = "" Then VGParametros.nombrefactura = "al_factura"
...
Sub LlenaPtoVta()
Dim rs As ADODB.Recordset
SQL = "select puntovtacodigo from vt_usuarioxpuntovta where usuariocodigo='" & txtUserName.text & "'"
Set rs = VGCNx.Execute(SQL)
SQL = "select a.puntovtacodigo,a.puntovtadescripcion,a.codigocajavtas from vt_puntoventa a"
If rs.RecordCount = 0 Then
Else
   SQL = SQL & " inner join vt_usuarioxpuntovta b on a.puntovtacodigo=b.puntovtacodigo "
   SQL = SQL & " where B.usuariocodigo='" & txtUserName.text & "'"
End If
Set rs = VGCNx.Execute(SQL)
VGParametros.listaPuntoVtas = ""
VGParametros.listacajas = ""
Combo4.Clear
Do While Not rs.EOF
    Combo4.AddItem rs(0) & " " & rs(1)
    VGParametros.listaPuntoVtas = VGParametros.listaPuntoVtas & rs(0) & ","
    VGParametros.listacajas = VGParametros.listacajas & rs(2) & ","
    
    rs.MoveNext
Loop
If Len(VGParametros.listaPuntoVtas) > 1 Then VGParametros.listaPuntoVtas = Left(VGParametros.listaPuntoVtas, Len(VGParametros.listaPuntoVtas) - 1)
If Len(VGParametros.listacajas) > 1 Then VGParametros.listacajas = Left(VGParametros.listacajas, Len(VGParametros.listacajas) - 1)
Combo4.ListIndex = 0
End Sub
