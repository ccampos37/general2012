...
'   VGgeneral.ConnectionString = "Provider=SQLOLEDB.1;Persist Security Info=False;User ID=pirata;Initial Catalog=MARFICE;Data Source=DESARROLLO"
   ...
   VGgeneral.Open
    ...
    VGcadenareport2 = "DSN=jckconsultores;DSQ=" & VGParamSistem.BDEmpresaGEN & ";UID=" & VGParamSistem.UsuarioGEN & ";PWD=" & DECODIFICA(Trim(VGParamSistem.PwdGEN), NUMMAGICO) & ""
    VGcadenareport2 = "DSN=jckconsultores;DSQ=" & VGParamSistem.BDEmpresaGEN & ";UID=" & VGParamSistem.UsuarioGEN & ";PWD=" & Trim(VGParamSistem.PwdGEN) & ""
  

      
   g_Ip = VGdllApi.LeerIni(App.Path & "\Marfice.ini", "CONEXION", "ip", "")
   If g_Ip = "" Then
      g_Ip = "127.0.0.1"
   End If
   Exit Sub
...
Public Function numero(Number) As String
   ...
   Else
     aValor = Number
   End If
   numero = Right(Space(12) & Trim(Format(aValor, "###,###0.00000")), 14)
End Function
