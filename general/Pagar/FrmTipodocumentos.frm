...
Private Sub cAcepta_Click()
 If adll.VerificaDatoExistente(VGCNx, "select * from cp_tipodocumento Where tdocumentocodigo='" & txt(0) & "'") = 1 And modoinsert = True Then
    MsgBox "Ya existe el Codigo...!!!", vbInformation, MsgTitle
    Exit Sub
 End If

 If modoinsert = True Then
       VGCNx.Execute "Insert Into cp_tipodocumento " & _
                  ...
                  "tdocumentoaplicadifcamb,tdocumentonotaconta,documentoretencion,tdocumentoactualizaxtesoreria," & _
                  "tdocumentosunat,usuariocodigo,fechaact,tdocumentocancela)" & _
                  "VALUES(" & _
                  "'" & txt(0) & "','" & txt(1) & "'," & _
                  "'" & txt(2) & "','" & txt(3) & "'," & _
 ...
 ElseIf modoedit = True Then
       VGCNx.Execute "Update cp_tipodocumento " & _
                  ...
                  "tdocumentoactualizaxtesoreria='" & IIf(chk(11).Value = 1, "1", "0") & "'," & _
...
Public Function Listado()
    ...
    Call adll.ListarEnTDBGRID(VGCNx, "cp_tipodocumento", TDBGrid1, "tdocumentocodigo,tdocumentodescripcion,tdocumentorenovarletras as Renovacion_Letras,tdocumentodocrenovaletra as Doc_Renova,tdocumentovalidabanco,tdocumentonumeauto", "tdocumentocodigo", nLongicampo)
    Call ConfiguraGrid
 ...
 modoinsert = False
  ...
  Select Case Index
     ...
     Case 1   'modificar
        ...
        Set rs = VGCNx.Execute("select * from cp_tipodocumento Where tdocumentocodigo='" & TDBGrid1.Columns(0).Text & "'")
        If rs.RecordCount > 0 Then
           ...
           chk(11).Value = IIf(Escadena(rs!tdocumentoactualizaxtesoreria) = "1", 1, 0)
     ...
     Case 2   'eliminar
          ...
          If MsgBox("Desea Eliminar el Registro?", vbYesNo, MsgTitle) = vbYes Then
              VGCNx.Execute "Delete From  cp_tipodocumento where tdocumentocodigo='" & TDBGrid1.Columns(0).Text & "'"
          End If
          Call Listado
     Case 3  'Imprimir
     aform(0) = "empresa='AA'"
     Aparam(0) = VGCNx.DefaultDatabase
     Call ImpresionRptProc("cp_manttipodocumento.rpt", aform, Aparam, , "Tipo de documentos")
'       Call ImpresionRptProc("cp_MantTipoDocumento.rpt", aform, aparam, , "tabla de documentos")
     Case 4  ' salir
       Unload Me
  End Select
...
Private Sub txt_LostFocus(Index As Integer)
   If Index = 6 Then
       txt(Index) = Right$("00000000000000" & txt(Index), txt(Index).MaxLength)
   ElseIf Index = 0 Then
       txt(Index) = Right$("00000000000000" & txt(Index), txt(Index).MaxLength)
