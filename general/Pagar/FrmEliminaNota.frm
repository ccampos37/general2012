...
Begin VB.Form xxxFrmEliminaNota 
   ...
   End
End
Attribute VB_Name = "xxxFrmEliminaNota"
...
Public Function GrabarData() As Integer
    ...
    If adll.VerificaDatoExistente(cn, "select * from vt_abono where documentoabono='" & adll.ComboDato(Combo2) & "' and  abononumdoc='" & Trim(MBox(1) & MBox(2)) & "'") = 0 Then
        ...
        cn.Execute " Delete From vt_pedido " & _
                    " where pedidotipofac='" & adll.ComboDato(Combo2) & "' and pedidonrofact='" & Trim(MBox(1) & MBox(2)) & "'"
                     
        cn.Execute " Delete From vt_cargo " & _
                   " where documentocargo='" & adll.ComboDato(Combo2) & "' and cargonumdoc='" & Trim(MBox(1) & MBox(2)) & "'"
                    
                    
        cn.Execute "insert into sysseguridad  values ('" & Date & "','" & Time & "','" & g_usuario & "','" & _
                    " Registro Eliminado de Notas de Ventas... Documento : " & adll.ComboDato(Combo2) & "-" & Trim(MBox(1) & MBox(2)) & _
                    " Cliente  : " & Escadena(Ctr_Ayuda1.xclave) & "- " & Trim(Escadena(Ctr_Ayuda1.xnombre)) & _
...
Private Sub cmdBotones_Click(Index As Integer)
   ...
   Select Case Index
    Case 11
        If MsgBox("Desea Eliminar el Documento?", vbInformation, MsgTitle) = vbNo Then
            Exit Sub
        End If
